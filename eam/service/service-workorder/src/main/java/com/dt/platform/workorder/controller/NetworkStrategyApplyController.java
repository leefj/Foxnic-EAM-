...
public class NetworkStrategyApplyController extends SuperController implements BpmCallbackController{
	...
	*/
	...
	public Result deleteByIds(List<String> ids) {
		...
		// 查询引用
		Map<String, Boolean> hasRefersMap = networkStrategyApplyService.hasRefers(ids);
		// 收集可以删除的ID值
	...
	*/
	...
	public Result<NetworkStrategyApply> getById(String id) {
		...
		// join 关联的对象
		networkStrategyApplyService.dao().fill(networkStrategyApply)
			.with("originator")
			.execute();
	...
	*/
	...
	public Result<PagedList<NetworkStrategyApply>> queryPagedList(NetworkStrategyApplyVO sample) {
		...
		// join 关联的对象
		networkStrategyApplyService.dao().fill(list)
			.with("originator")
			.execute();
		// 填充流程相关的属性
