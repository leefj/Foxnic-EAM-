...
public class RecruitmentPlanApplyBpmEventAdaptor extends BpmEventAdaptor<RecruitmentPlanApply,IRecruitmentPlanApplyService> {

	public String BPM_TABLE="hr_recruitment_plan_apply";

	private void updateBillStatus(String status ,String id){
		this.dao().execute("update "+BPM_TABLE+" set status=? where id=?", status, id);
	}

	public RecruitmentPlanApplyBpmEventAdaptor(IRecruitmentPlanApplyService service) {
		super(service);
	}
