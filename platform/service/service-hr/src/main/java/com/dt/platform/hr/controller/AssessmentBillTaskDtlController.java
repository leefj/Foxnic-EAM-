...
public class AssessmentBillTaskDtlController extends SuperController {
    ...
    @ApiImplicitParams({
		...
		@ApiImplicitParam(name = AssessmentBillTaskDtlVOMeta.RELATIONSHIP, value = "考核方式", required = false, dataTypeClass = String.class, example = "self"),
		...
		@ApiImplicitParam(name = AssessmentBillTaskDtlVOMeta.BILL_ID, value = "任务", required = false, dataTypeClass = String.class)
	})
    ...
    public Result<AssessmentBillTaskDtl> getById(String id) {
        ...
        // join 关联的对象
        assessmentBillTaskDtlService.dao().fill(assessmentBillTaskDtl).with(AssessmentBillTaskDtlMeta.ASSESSMENT_BILL_TASK).execute();
    ...
    public Result<PagedList<AssessmentBillTaskDtl>> queryPagedList(AssessmentBillTaskDtlVO sample) {
        ...
        // join 关联的对象
        assessmentBillTaskDtlService.dao().fill(list).with(AssessmentBillTaskDtlMeta.ASSESSOR).with(AssessmentBillTaskDtlMeta.ASSESSMENT_BILL_TASK).with(AssessmentBillTaskDtlMeta.WITH_LEADER_LIST).with(AssessmentBillTaskDtlMeta.WITH_SAME_USER_LIST).with(AssessmentBillTaskDtlMeta.WITH_SECOND_LEADER_LIST).with(AssessmentBillTaskDtlMeta.WITH_SELF_USER).execute();

        List<Employee> assessorList = CollectorUtil.collectList(list, AssessmentBillTaskDtl::getAssessor);
        assessmentBillTaskDtlService.dao().join(assessorList, Person.class);
