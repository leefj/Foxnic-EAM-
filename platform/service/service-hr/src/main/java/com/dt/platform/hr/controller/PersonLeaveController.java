...
public class PersonLeaveController extends SuperController implements BpmCallbackController {
    ...
    @ApiImplicitParams({
		...
		@ApiImplicitParam(name = PersonLeaveVOMeta.STATUS, value = "办理状态", required = false, dataTypeClass = String.class),
    ...
    public Result<PersonLeave> getById(String id) {
        ...
        // join 关联的对象
        personLeaveService.dao().fill(personLeave).with("person").with("handover").execute();

        personLeaveService.dao().join(personLeave.getPerson(), Person.class);
        personLeaveService.dao().join(personLeave.getHandover(), Person.class);
    ...
    public Result<PagedList<PersonLeave>> queryPagedList(PersonLeaveVO sample) {
        ...
        // join 关联的对象
        personLeaveService.dao().fill(list).with("person").with("handover").execute();

        List<Employee> employees = CollectorUtil.collectList(list, PersonLeave::getPerson);
        personLeaveService.dao().join(employees, Person.class);

        List<Employee> employees2 = CollectorUtil.collectList(list, PersonLeave::getHandover);
        personLeaveService.dao().join(employees2, Person.class);
