...
public class PersonLeaveBpmEventAdaptor extends BpmEventAdaptor<PersonLeave,IPersonLeaveService> {

	public String BPM_TABLE="hr_person_leave";

	private void updateBillStatus(String status ,String id){
		this.dao().execute("update "+BPM_TABLE+" set status=? where id=?", status, id);
	}
	public PersonLeaveBpmEventAdaptor(IPersonLeaveService service) {
		super(service);
	}
