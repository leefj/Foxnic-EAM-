...
public class PersonAbsenceApplyBpmEventAdaptor extends BpmEventAdaptor<PersonAbsenceApply,IPersonAbsenceApplyService> {

	public String BPM_TABLE="hr_person_absence_apply";

	private void updateBillStatus(String status ,String id){
		this.dao().execute("update "+BPM_TABLE+" set status=? where id=?", status, id);
	}

	public PersonAbsenceApplyBpmEventAdaptor(IPersonAbsenceApplyService service) {
		super(service);
	}
