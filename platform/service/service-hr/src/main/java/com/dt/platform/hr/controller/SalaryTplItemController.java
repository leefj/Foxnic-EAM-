...
public class SalaryTplItemController extends SuperController {
    ...
    @ApiImplicitParams({
		...
		@ApiImplicitParam(name = SalaryTplItemVOMeta.SORT, value = "计算顺序", required = false, dataTypeClass = Integer.class),
    ...
    public Result<SalaryTplItem> getById(String id) {
        ...
        // join 关联的对象
        salaryTplItemService.dao().fill(salaryTplItem).with(SalaryTplItemMeta.SALARY_COLUMN).execute();
    ...
    public Result<PagedList<SalaryTplItem>> queryPagedList(SalaryTplItemVO sample) {
        ...
        ob.asc("sort");
        PagedList<SalaryTplItem> list = salaryTplItemService.queryPagedList(sample,ob, sample.getPageSize(), sample.getPageIndex());
        // join 关联的对象
        salaryTplItemService.dao().fill(list).with(SalaryTplItemMeta.SALARY_COLUMN).execute();
        result.success(true).data(list);
        return result;
    }
}
