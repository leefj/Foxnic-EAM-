...
public class PersonOfficialBusinessController extends SuperController implements BpmCallbackController {
    ...
    @ApiImplicitParams({
		...
		@ApiImplicitParam(name = PersonOfficialBusinessVOMeta.STATUS, value = "办理状态", required = false, dataTypeClass = String.class),
    ...
    public Result<PersonOfficialBusiness> getById(String id) {
        ...
        // join 关联的对象
        personOfficialBusinessService.dao().fill(personOfficialBusiness).with("person").execute();

        personOfficialBusinessService.dao().join(personOfficialBusiness.getPerson(), Person.class);
    ...
    public Result<PagedList<PersonOfficialBusiness>> queryPagedList(PersonOfficialBusinessVO sample) {
        ...
        // join 关联的对象
        personOfficialBusinessService.dao().fill(list).with("person").execute();

        List<Employee> employees = CollectorUtil.collectList(list, PersonOfficialBusiness::getPerson);
        personOfficialBusinessService.dao().join(employees, Person.class);
