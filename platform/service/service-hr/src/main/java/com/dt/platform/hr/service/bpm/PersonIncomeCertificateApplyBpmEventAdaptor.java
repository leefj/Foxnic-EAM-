...
public class PersonIncomeCertificateApplyBpmEventAdaptor extends BpmEventAdaptor<PersonIncomeCertificateApply,IPersonIncomeCertificateApplyService> {

	public String BPM_TABLE="hr_person_income_certificate_apply";
	private void updateBillStatus(String status ,String id){
		this.dao().execute("update "+BPM_TABLE+" set status=? where id=?", status, id);
	}

	public PersonIncomeCertificateApplyBpmEventAdaptor(IPersonIncomeCertificateApplyService service) {
		super(service);
	}
