...
public class PersonnelRequirementApplyBpmEventAdaptor extends BpmEventAdaptor<PersonnelRequirementApply,IPersonnelRequirementApplyService> {

	public String BPM_TABLE="hr_personnel_requirement_apply";
	private void updateBillStatus(String status ,String id){
		this.dao().execute("update "+BPM_TABLE+" set status=? where id=?", status, id);
	}

	public PersonnelRequirementApplyBpmEventAdaptor(IPersonnelRequirementApplyService service) {
		super(service);
	}
