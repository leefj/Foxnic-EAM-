...
public class PersonAbsenceApplyController extends SuperController implements BpmCallbackController {
    ...
    @ApiImplicitParams({
		...
		@ApiImplicitParam(name = PersonAbsenceApplyVOMeta.STATUS, value = "办理状态", required = false, dataTypeClass = String.class),
    ...
    public Result<PersonAbsenceApply> getById(String id) {
        ...
        // join 关联的对象
        personAbsenceApplyService.dao().fill(personAbsenceApply).with("person").with(PersonAbsenceApplyMeta.ATTENDANCE_TYPE).execute();
        // 填充流程相关的属性
    ...
    public Result<PagedList<PersonAbsenceApply>> queryPagedList(PersonAbsenceApplyVO sample) {
        ...
        // join 关联的对象
        personAbsenceApplyService.dao().fill(list).with("person").with(PersonAbsenceApplyMeta.ATTENDANCE_TYPE).execute();
        // 填充流程相关的属性
