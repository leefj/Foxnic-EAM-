...
public class SalaryServiceImpl extends SuperService<Salary> implements ISalaryService {
	 ...
	 */
	@Override
	public Result insert(Salary salary,boolean throwsException) {
		Result r=super.insert(salary,throwsException);
		return r;
	}
	...
	public List<ValidateResult> importData(RcdSet rs) {
		...
		for(int i=0;i<rs.getRcdList().size();i++){
			...
			if(StringUtil.isBlank(id)){
				errors.add(new ValidateResult(null,(i+1),"未找到ID主键"));
				break;
			}
			...
			//处理status
			String personalStatusName=r.getString("personal_status_name");
			if(StringUtil.isBlank(personalStatusName)){
				update.set("personal_status","");
			}else{
				CodeTextEnum vStatus= EnumUtil.parseByText(StatusEnableEnum.class,personalStatusName);
				update.set("personal_status",vStatus.code());
			}
	 ...
	 * */
	@Override
	public Result insert(Salary salary) {
		return this.insert(salary,true);
	}

	/**
	 * 批量插入实体，事务内
	 * @param salaryList 实体数据清单
	 * @return 插入是否成功
	 * */
	@Override
	public Result insertList(List<Salary> salaryList) {
		return super.insertList(salaryList);
	}
	...
	public Result deleteByIdPhysical(String id) {
		...
		salary.setId(id);
		try {
			boolean suc = dao.deleteEntity(salary);
	...
	public Result deleteByIdLogical(String id) {
		...
		salary.setDeleteBy((String)dao.getDBTreaty().getLoginUserId());
		salary.setDeleteTime(new Date());
		try {
			boolean suc = dao.updateEntity(salary,SaveMode.NOT_NULL_FIELDS);
			return suc?ErrorDesc.success():ErrorDesc.failure();
		}
		catch(Exception e) {
			Result r= ErrorDesc.failure();
	 ...
	 * */
	@Override
	public Result update(Salary salary , SaveMode mode) {
		return this.update(salary,mode,true);
	}
	 ...
	 * */
	@Override
	public Result update(Salary salary , SaveMode mode,boolean throwsException) {
		Result r=super.update(salary , mode , throwsException);
		return r;
	}

	/**
	 * 更新实体集，事务内
	 * @param salaryList 数据对象列表
	 * @param mode 保存模式
	 * @return 保存是否成功
	 * */
	@Override
	public Result updateList(List<Salary> salaryList , SaveMode mode) {
		return super.updateList(salaryList , mode);
	}
	 ...
	 * */
	@Override
	public List<Salary> getByIds(List<String> ids) {
		return this.queryListByIds(ids);
	}

	@Override
	public List<Salary> queryListByIds(List<String> ids) {
		return super.queryListByUKeys("id",ids);
	}

	@Override
	public Map<String, Salary> queryMapByIds(List<String> ids) {
		return super.queryMapByUKeys("id",ids, Salary::getId);
	}
	 ...
	 * */
	@Override
	public List<Salary> queryList(SalaryVO sample) {
	 ...
	 * */
	@Override
	public PagedList<Salary> queryPagedList(SalaryVO sample, int pageSize, int pageIndex) {
	 ...
	 * */
	@Override
	public PagedList<Salary> queryPagedList(Salary sample, ConditionExpr condition, int pageSize, int pageIndex) {
	 ...
	 * @return 判断结果
	 */
	public Boolean checkExists(Salary salary) {
		//TDOD 此处添加判断段的代码
		//boolean exists=super.checkExists(salary, SYS_ROLE.NAME);
		//return exists;
		return false;
	}
