...
public class PersonConfirmApplyBpmEventAdaptor extends BpmEventAdaptor<PersonConfirmApply,IPersonConfirmApplyService> {

	public String BPM_TABLE="hr_person_confirm_apply";

	private void updateBillStatus(String status ,String id){
		this.dao().execute("update "+BPM_TABLE+" set status=? where id=?", status, id);
	}

	public PersonConfirmApplyBpmEventAdaptor(IPersonConfirmApplyService service) {
		super(service);
	}
