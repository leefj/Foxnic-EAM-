...
public class PersonConfirmApplyController extends SuperController implements BpmCallbackController{
	...
	*/
	...
	@ApiImplicitParams({
		...
		@ApiImplicitParam(name = PersonConfirmApplyVOMeta.STATUS , value = "状态" , required = false , dataTypeClass=String.class),
	...
	*/
	...
	public Result<PersonConfirmApply> getById(String id) {
		...
		// join 关联的对象
		personConfirmApplyService.dao().fill(personConfirmApply)
			.with("person")
			.execute();
		// 填充流程相关的属性
	...
	*/
	...
	public Result<PagedList<PersonConfirmApply>> queryPagedList(PersonConfirmApplyVO sample) {
		...
		// join 关联的对象
		personConfirmApplyService.dao().fill(list)
			.with("person")
			.execute();
		// 填充流程相关的属性
