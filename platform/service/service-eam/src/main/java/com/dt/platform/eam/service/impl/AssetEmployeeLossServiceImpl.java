...
public class AssetEmployeeLossServiceImpl extends SuperService<AssetEmployeeLoss> implements IAssetEmployeeLossService {
	...
	public Result update(AssetEmployeeLoss assetEmployeeLoss , SaveMode mode,boolean throwsException) {
		...
		if(r.success()){
			...
			dao.execute("delete from eam_asset_item where crd ='d'  and  handle_id=?",assetEmployeeLoss.getId());
		}
		return r;
	}
	...
	public  BpmActionResult onProcessCallback(BpmEvent event) {
		...
		try {
			if(event!=null&&event.getProcessInstance()!=null&&event.getProcessInstance().getTitle()!=null){
				dao.execute("update eam_asset_employee_loss set name=? where id=?",event.getProcessInstance().getTitle(),event.getBillId());
			}
