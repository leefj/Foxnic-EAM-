...
public class CCustRepairApplyController extends SuperController {
	...
	*/
	...
	@ApiImplicitParams({
		...
		@ApiImplicitParam(name = CCustRepairApplyVOMeta.PLAN_FINISH_DATE , value = "预期日期" , required = false , dataTypeClass=Date.class),
		...
		@ApiImplicitParam(name = CCustRepairApplyVOMeta.FINISH_DATE , value = "结束时间" , required = false , dataTypeClass=Date.class),
	})
	...
	*/
	...
	public Result<CCustRepairApply> getById(String id) {
		...
		// join 关联的对象
		cCustRepairApplyService.dao().fill(cCustRepairApply)
			.with("processUser")
				.with(CCustRepairApplyMeta.REPIAR_ITEM_DATA)
			.execute();

		List<CCustRepiarItem> list=cCustRepairApply.getRepiarItemData();
		cCustRepairApplyService.dao().fill(list)
				.with("processUser")
				.execute();
