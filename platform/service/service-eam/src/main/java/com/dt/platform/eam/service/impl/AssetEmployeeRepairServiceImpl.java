...
public class AssetEmployeeRepairServiceImpl extends SuperService<AssetEmployeeRepair> implements IAssetEmployeeRepairService {
	...
	public Result update(AssetEmployeeRepair assetEmployeeRepair , SaveMode mode,boolean throwsException) {
		...
		if(r.success()){
			...
			dao.execute("delete from eam_asset_item where crd ='d'  and  handle_id=?",assetEmployeeRepair.getId());
		}
		return r;
	}
	...
	public  BpmActionResult onProcessCallback(BpmEvent event) {
		...
		try {
			if(event!=null&&event.getProcessInstance()!=null&&event.getProcessInstance().getTitle()!=null){
				dao.execute("update eam_asset_employee_repair set name=? where id=?",event.getProcessInstance().getTitle(),event.getBillId());
			}
