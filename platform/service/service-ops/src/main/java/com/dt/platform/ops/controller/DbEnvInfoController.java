...
public class DbEnvInfoController extends SuperController {
    ...
    public Result<DbEnvInfo> getById(String id) {
        ...
        // join 关联的对象
        dbEnvInfoService.dao().fill(dbEnvInfo).with("host").with("dbInfo").execute();
    ...
    public Result<PagedList<DbEnvInfo>> queryPagedList(DbEnvInfoVO sample) {
        ...
        // join 关联的对象
        dbEnvInfoService.dao().fill(list).with(DbEnvInfoMeta.CIPHERTEXT_BOX_DATA).with("host").with("dbInfo").with("dbInfo").execute();
