...
public class DbExtractApplyController extends SuperController {
    ...
    @ApiImplicitParams({
		...
		@ApiImplicitParam(name = DbExtractApplyVOMeta.ITEM_CODE, value = "关联工单", required = false, dataTypeClass = String.class),
    ...
    public Result<DbExtractApply> getById(String id) {
        ...
        // join 关联的对象
        dbExtractApplyService.dao().fill(dbExtractApply).with("applyUser").with(DbExtractApplyMeta.DB_INFO_APPLY).with(DbExtractApplyMeta.REQUEST_ORDER).execute();
    ...
    public Result<PagedList<DbExtractApply>> queryPagedList(DbExtractApplyVO sample) {
        ...
        // join 关联的对象
        dbExtractApplyService.dao().fill(list).with(DbExtractApplyMeta.APPLY_USER).with(DbExtractApplyMeta.DB_INFO_APPLY).with(DbExtractApplyMeta.REQUEST_ORDER).execute();

        List<Employee> users = CollectorUtil.collectList(list, DbExtractApply::getApplyUser);
        dbExtractApplyService.dao().join(users, Person.class);
        result.success(true).data(list);
        return result;
    }
}
