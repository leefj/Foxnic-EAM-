...
public class SoftwareInfoController extends SuperController {
	...
	*/
	...
	@NotNull(name = SoftwareInfoVOMeta.ID)
	...
	*/
	...
	@NotNull(name = SoftwareInfoVOMeta.IDS)
	...
	*/
	...
	public Result<SoftwareInfo> getById(String id) {
		...
		// join 关联的对象
		softwareInfoService.dao().fill(softwareInfo)
			.with(SoftwareInfoMeta.GROUP)
			.execute();
	...
	*/
	...
	public Result<PagedList<SoftwareInfo>> queryPagedList(SoftwareInfoVO sample) {
		...
		// join 关联的对象
		softwareInfoService.dao().fill(list)
			.with(SoftwareInfoMeta.GROUP)
			.execute();
		result.success(true).data(list);
		return result;
