...
public class PageInfoController extends SuperController {
    ...
    @ApiImplicitParams({
		...
		@ApiImplicitParam(name = PageInfoVOMeta.LABEL_CODE, value = "标签", required = false, dataTypeClass = String.class)
	})
    ...
    @ApiOperationSupport(order=2 , author="金杰 , maillank@qq.com")
    @SentinelResource(value = PageInfoServiceProxy.DELETE , blockHandlerClass = { SentinelExceptionUtil.class } , blockHandler = SentinelExceptionUtil.HANDLER )
    @PostMapping(PageInfoServiceProxy.DELETE)
    public Result deleteById(String id) {
        ...
        // 引用校验
        ReferCause cause =  pageInfoService.hasRefers(id);
        // 判断是否可以删除
        this.validator().asserts(cause.hasRefer()).requireEqual("不允许删除当前记录："+cause.message(),false);
        if(this.validator().failure()) {
            return this.validator().getFirstResult().messageLevel4Confirm();
        }
        Result result=pageInfoService.deleteByIdLogical(id);
        return result;
    }
    ...
    public Result<PageInfo> getById(String id) {
        ...
        pageInfoService.dao().fill(pageInfo).with(PageInfoMeta.LABEL_DICT).execute();
    ...
    public Result<PagedList<PageInfo>> queryPagedList(PageInfoVO sample) {
        ...
        pageInfoService.dao().fill(list).with(PageInfoMeta.LABEL_DICT).execute();
