...
public class ScreenController extends SuperController {
	...
	@ApiImplicitParams({
			...
			@ApiImplicitParam(name = ScreenVOMeta.STATUS, value = "状态", required = false, dataTypeClass = String.class),
			@ApiImplicitParam(name = ScreenVOMeta.NOTES, value = "备注", required = false, dataTypeClass = String.class),
			@ApiImplicitParam(name = ScreenVOMeta.JSON_DATA, value = "大屏定义", required = false, dataTypeClass = String.class)
	})
	 ...
	 * 复制
	 */
	@ApiOperation(value = "复制大屏")
	@ApiImplicitParams({
			@ApiImplicitParam(name = ScreenVOMeta.ID, value = "主键", required = true, dataTypeClass = String.class),})
	@ApiOperationSupport(order = 8, author = "金杰 , maillank@qq.com")
	@SentinelResource(value = ScreenServiceProxy.COPY_DATA, blockHandlerClass = { SentinelExceptionUtil.class }, blockHandler = SentinelExceptionUtil.HANDLER)
	@PostMapping(ScreenServiceProxy.COPY_DATA)
	public Result copyData(String id) {
		return screenService.copyData(id);
	}

	/**
	 * 分页查询大屏
	 */
	@ApiOperation(value = "应用大屏")
	@ApiImplicitParams({
			@ApiImplicitParam(name = ScreenVOMeta.ID, value = "主键", required = true, dataTypeClass = String.class),})
	@ApiOperationSupport(order = 8, author = "金杰 , maillank@qq.com")
	@SentinelResource(value = ScreenServiceProxy.APPLY, blockHandlerClass = { SentinelExceptionUtil.class }, blockHandler = SentinelExceptionUtil.HANDLER)
	@PostMapping(ScreenServiceProxy.APPLY)
	public Result apply(String id) {
		return screenService.apply(id);
	}
