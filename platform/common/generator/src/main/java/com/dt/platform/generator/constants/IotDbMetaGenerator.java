...
public class IotDbMetaGenerator {
	...
	public static void main(String[] args) throws Exception {
		IotDbMetaGenerator g = new IotDbMetaGenerator();
		g.buildDBMeta();
	}
	...
	private void buildDBMeta() {
		...
		// 主文件
		DBMetaClassFile dbMetaBuilder=new DBMetaClassFile(dao,configs.getDomianProject(),this.configs.getProjectConfigs().getDomainConstantsPackage(),"IotTables");
		dbMetaBuilder.setTableFilter(table->{
			table=table.toLowerCase();
			if(
					table.startsWith("iot_") ) return true;
			return false;
		});
