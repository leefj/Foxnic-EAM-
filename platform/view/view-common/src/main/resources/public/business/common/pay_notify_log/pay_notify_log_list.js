...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-common/sys-pay-notify-log";
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'taskId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('通知任务编号'), templet: function (d) { return templet('taskId' ,fox.joinLabel(d.payNotifyTask,"id",',','','taskId'),d);}}
					,{ field: 'notifyTimes', align:"right",fixed:false,  hide:false, sort: true  , title: fox.translate('第几次被通知') , templet: function (d) { return templet('notifyTimes',d.notifyTimes,d);}  }
					,{ field: 'response', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('请求参数') , templet: function (d) { return templet('response',d.response,d);}  }
	...
	function refreshTableData(sortField,sortType,reset) {
		...
		value.taskId={ inputType:"select_box", value: getSelectedValue("#taskId","value") ,fillBy:["payNotifyTask"]  , label:getSelectedValue("#taskId","nameStr") };
		var ps={searchField:"$composite"};
		if(window.pageExt.list.beforeQuery){
		...
		fox.renderSelectBox({
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					if(!data[i]) continue;
					if(window.pageExt.list.selectBoxDataTransform) {
						opts.push(window.pageExt.list.selectBoxDataTransform("taskId",{data:data[i],name:data[i].id,value:data[i].id},data[i],data,i));
					} else {
						opts.push({data:data[i],name:data[i].id,value:data[i].id});
					}
				}
				return opts;
			}
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('sys-pay-notify-log-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('sys-pay-notify-log-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'支付通知日志'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'支付通知日志'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('sys-pay-notify-log-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				top.layer.load(2);
				top.layer.load(2);
				admin.post(getByIdURL, { id : data.id }, function (data) {
					...
					if(data.success) {
						admin.putTempData('sys-pay-notify-log-form-data-form-action', "edit",true);
				...
				admin.post(getByIdURL, { id : data.id }, function (data) {
					top.layer.closeAll('loading');
					if(data.success) {
						admin.putTempData('sys-pay-notify-log-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
				...
				top.layer.confirm(fox.translate('确定删除此'+'支付通知日志'+'吗？'), function (i) {
					top.layer.close(i);
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('sys-pay-notify-log-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('sys-pay-notify-log-form-data', data);
		var area=admin.getTempData('sys-pay-notify-log-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('支付通知日志');
		...
		admin.popupCenter({
			...
			area: ["80%",height+"px"],
			type: 2,
			id:"sys-pay-notify-log-form-data-win",
			content: '/business/common/pay_notify_log/pay_notify_log_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
