...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('sys-user-import-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('sys-user-import-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'sys-user-import-form-data-win');
			if(area==null) return;
			admin.putTempData('sys-user-import-form-area', area);
	...
	function renderFormFields() {
		...
		laydate.render({
			...
			trigger:"click",
				...
				window.pageExt.form.onDatePickerChanged && window.pageExt.form.onDatePickerChanged("actionOperTime",value, date, endDate);
			}
		});
		form.on('radio(actionStatus)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=actionStatus]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("actionStatus",data,checked);
		});
	}
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('sys-user-import-form-data');
		}
		...
		if (hasData) {
			...
			//设置 操作时间 显示复选框勾选
			if(formData["actionOperTime"]) {
				$("#actionOperTime").val(fox.dateFormat(formData["actionOperTime"],"yyyy-MM-dd HH:mm:ss"));
			}
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('sys-user-import-form-data');
		}
		return rawFormData;
	}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('sys-user-import-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('sys-user-import-form-data-win',this); });
