...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-common/sys-screen-ds-db";
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'dsType', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('数据库类型'), templet:function (d){ return templet('dsType',fox.getEnumText(RADIO_DSTYPE_DATA,d.dsType,'','dsType'),d);}}
					,{ field: 'user', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('账户') , templet: function (d) { return templet('user',d.user,d);}  }
					,{ field: 'pwd', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('密码') , templet: function (d) { return templet('pwd',d.pwd,d);}  }
					,{ field: 'uri', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('uri') , templet: function (d) { return templet('uri',d.uri,d);}  }
	...
	function refreshTableData(sortField,sortType,reset) {
		...
		value.dsType={ inputType:"radio_box", value: getSelectedValue("#dsType","value"), label:getSelectedValue("#dsType","nameStr") };
		var ps={searchField:"$composite"};
		if(window.pageExt.list.beforeQuery){
	...
	function initSearchFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.list.onSelectBoxChanged && window.pageExt.list.onSelectBoxChanged("dsType",data.arr,data.change,data.isAdd);
				},1);
			},
			...
			transform:function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					if(window.pageExt.list.selectBoxDataTransform) {
						opts.push(window.pageExt.list.selectBoxDataTransform("dsType",{data:data[i],name:data[i].text,value:data[i].code},data[i],data,i));
					} else {
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('sys-screen-ds-db-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('sys-screen-ds-db-form-data-form-action', "create",true);
            showEditForm(data);
        };
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('sys-screen-ds-db-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				top.layer.load(2);
				top.layer.load(2);
				admin.post(getByIdURL, { id : data.id }, function (data) {
					...
					if(data.success) {
						admin.putTempData('sys-screen-ds-db-form-data-form-action', "edit",true);
				...
				admin.post(getByIdURL, { id : data.id }, function (data) {
					top.layer.closeAll('loading');
					if(data.success) {
						admin.putTempData('sys-screen-ds-db-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('sys-screen-ds-db-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('sys-screen-ds-db-form-data', data);
		var area=admin.getTempData('sys-screen-ds-db-form-area');
		...
		admin.popupCenter({
			...
			area: ["80%",height+"px"],
			type: 2,
			id:"sys-screen-ds-db-form-data-win",
			content: '/business/common/screen_ds_db/screen_ds_db_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
