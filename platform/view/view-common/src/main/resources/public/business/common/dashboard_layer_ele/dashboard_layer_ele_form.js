...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('sys-dashboard-layer-ele-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('sys-dashboard-layer-ele-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'sys-dashboard-layer-ele-form-data-win');
			if(area==null) return;
			admin.putTempData('sys-dashboard-layer-ele-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			el: "layerId",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("面板",'','cmp:form'),
			filterable: true,
			layVerify: 'required',
			layVerType: 'msg',
			on: function(data){
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					if(window.pageExt.form.selectBoxDataTransform) {
						opts.push(window.pageExt.form.selectBoxDataTransform("layerId",{data:data[i],name:data[i].name,value:data[i].id,selected:(defaultValues.indexOf(data[i].id)!=-1 || defaultIndexs.indexOf(""+i)!=-1)},data[i],data,i));
					} else {
		...
		fox.renderSelectBox({
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					if(!data[i]) continue;
					if(window.pageExt.form.selectBoxDataTransform) {
						opts.push(window.pageExt.form.selectBoxDataTransform("source",{data:data[i],name:data[i].name,value:data[i].id,selected:(defaultValues.indexOf(data[i].id)!=-1 || defaultIndexs.indexOf(""+i)!=-1)},data[i],data,i));
					} else {
						opts.push({data:data[i],name:data[i].name,value:data[i].id,selected:(defaultValues.indexOf(data[i].id)!=-1 || defaultIndexs.indexOf(""+i)!=-1)});
					}
				}
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('sys-dashboard-layer-ele-form-data');
		}
		...
		if (hasData) {
			...
			//设置  面板 设置下拉框勾选
			fox.setSelectValue4QueryApi("#layerId",formData.dashboardLayer);
			//设置  类型 设置下拉框勾选
			fox.setSelectValue4Enum("#type",formData.type,SELECT_TYPE_DATA);
			//设置  来源 设置下拉框勾选
			fox.setSelectValue4QueryApi("#source",formData.reportChart);
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('sys-dashboard-layer-ele-form-data');
		}
		return rawFormData;
	}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('sys-dashboard-layer-ele-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('sys-dashboard-layer-ele-form-data-win',this); });
