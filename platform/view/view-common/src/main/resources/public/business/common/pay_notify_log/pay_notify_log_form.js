...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('sys-pay-notify-log-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('sys-pay-notify-log-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'sys-pay-notify-log-form-data-win');
			if(area==null) return;
			admin.putTempData('sys-pay-notify-log-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			el: "taskId",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("通知任务编号",'','cmp:form'),
			filterable: true,
			paging: true,
			pageRemote: true,
			layVerify: 'required',
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					if(window.pageExt.form.selectBoxDataTransform) {
						opts.push(window.pageExt.form.selectBoxDataTransform("taskId",{data:data[i],name:data[i].id,value:data[i].id,selected:(defaultValues.indexOf(data[i].id)!=-1 || defaultIndexs.indexOf(""+i)!=-1)},data[i],data,i));
					} else {
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('sys-pay-notify-log-form-data');
		}
		...
		if (hasData) {
			...
			//设置  通知任务编号 设置下拉框勾选
			fox.setSelectValue4QueryApi("#taskId",formData.payNotifyTask);
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('sys-pay-notify-log-form-data');
		}
		return rawFormData;
	}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('sys-pay-notify-log-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('sys-pay-notify-log-form-data-win',this); });
