...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('sys-pay-channel-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('sys-pay-channel-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'sys-pay-channel-form-data-win');
			if(area==null) return;
			admin.putTempData('sys-pay-channel-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			el: "merchantId",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("商户",'','cmp:form'),
			filterable: true,
			paging: true,
			pageRemote: true,
			on: function(data){
		...
		fox.renderSelectBox({
			el: "appId",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("应用",'','cmp:form'),
			filterable: true,
			paging: true,
			pageRemote: true,
			on: function(data){
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('sys-pay-channel-form-data');
		}
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('sys-pay-channel-form-data');
		}
		return rawFormData;
	}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('sys-pay-channel-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('sys-pay-channel-form-data-win',this); });
