...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('sys-form-data-ext-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('sys-form-data-ext-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'sys-form-data-ext-form-data-win');
			if(area==null) return;
			admin.putTempData('sys-form-data-ext-form-area', area);
	...
	function renderFormFields() {
		...
		laydate.render({
			...
			trigger:"click",
				...
				window.pageExt.form.onDatePickerChanged && window.pageExt.form.onDatePickerChanged("d1",value, date, endDate);
			}
		});
		laydate.render({
			elem: '#d2',
			...
			trigger:"click",
				...
				window.pageExt.form.onDatePickerChanged && window.pageExt.form.onDatePickerChanged("d2",value, date, endDate);
			}
		});
		laydate.render({
			elem: '#d3',
			...
			trigger:"click",
				...
				window.pageExt.form.onDatePickerChanged && window.pageExt.form.onDatePickerChanged("d3",value, date, endDate);
			}
		});
		laydate.render({
			elem: '#d4',
			...
			trigger:"click",
				...
				window.pageExt.form.onDatePickerChanged && window.pageExt.form.onDatePickerChanged("d4",value, date, endDate);
			}
		});
		laydate.render({
			elem: '#d5',
			...
			trigger:"click",
				...
				window.pageExt.form.onDatePickerChanged && window.pageExt.form.onDatePickerChanged("d5",value, date, endDate);
			}
		});
		laydate.render({
			elem: '#d6',
			...
			trigger:"click",
				...
				window.pageExt.form.onDatePickerChanged && window.pageExt.form.onDatePickerChanged("d6",value, date, endDate);
			}
		});
	}
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('sys-form-data-ext-form-data');
		}
		...
		if (hasData) {
			...
			//设置 时间1 显示复选框勾选
			if(formData["d1"]) {
				$("#d1").val(fox.dateFormat(formData["d1"],"yyyy-MM-dd HH:mm:ss"));
			}
			//设置 时间2 显示复选框勾选
			if(formData["d2"]) {
				$("#d2").val(fox.dateFormat(formData["d2"],"yyyy-MM-dd HH:mm:ss"));
			}
			//设置 时间3 显示复选框勾选
			if(formData["d3"]) {
				$("#d3").val(fox.dateFormat(formData["d3"],"yyyy-MM-dd HH:mm:ss"));
			}
			//设置 时间4 显示复选框勾选
			if(formData["d4"]) {
				$("#d4").val(fox.dateFormat(formData["d4"],"yyyy-MM-dd HH:mm:ss"));
			}
			//设置 时间5 显示复选框勾选
			if(formData["d5"]) {
				$("#d5").val(fox.dateFormat(formData["d5"],"yyyy-MM-dd HH:mm:ss"));
			}
			//设置 时间6 显示复选框勾选
			if(formData["d6"]) {
				$("#d6").val(fox.dateFormat(formData["d6"],"yyyy-MM-dd HH:mm:ss"));
			}
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('sys-form-data-ext-form-data');
		}
		return rawFormData;
	}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('sys-form-data-ext-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('sys-form-data-ext-form-data-win',this); });
