...
function FormPage() {
	...
	// 接口地址
	const moduleURL="/service-common/sys-dashboard-layer";
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('sys-dashboard-layer-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('sys-dashboard-layer-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'sys-dashboard-layer-form-data-win');
			if(area==null) return;
			admin.putTempData('sys-dashboard-layer-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			el: "colType",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("列数",'','cmp:form'),
			filterable: false,
			layVerify: 'required',
			layVerType: 'msg',
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("colType",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			transform:function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					if(window.pageExt.form.selectBoxDataTransform) {
						opts.push(window.pageExt.form.selectBoxDataTransform("colType",{data:data[i],name:data[i].text,value:data[i].code,selected:(defaultValues.indexOf(data[i].code)!=-1 || defaultIndexs.indexOf(""+i)!=-1)},data[i],data,i));
					} else {
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('sys-dashboard-layer-form-data');
		}
		...
		if (hasData) {
			...
			//设置  列数 设置下拉框勾选
			fox.setSelectValue4Enum("#colType",formData.colType,SELECT_COLTYPE_DATA);
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('sys-dashboard-layer-form-data');
		}
		return rawFormData;
	}
	...
	function getFormData() {
		...
		//获取 列数 下拉框的值
		data["colType"]=fox.getSelectedValue("colType",false);
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('sys-dashboard-layer-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('sys-dashboard-layer-form-data-win',this); });
