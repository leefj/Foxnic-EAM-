...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('ops-monitor-alert-log-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('ops-monitor-alert-log-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'ops-monitor-alert-log-form-data-win');
			if(area==null) return;
			admin.putTempData('ops-monitor-alert-log-form-area', area);
	...
	function renderFormFields() {
		...
		form.on('radio(actionResultStatus)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=actionResultStatus]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("actionResultStatus",data,checked);
		});
	}
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('ops-monitor-alert-log-form-data');
		}
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('ops-monitor-alert-log-form-data');
		}
		return rawFormData;
	}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('ops-monitor-alert-log-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('ops-monitor-alert-log-form-data-win',this); });
