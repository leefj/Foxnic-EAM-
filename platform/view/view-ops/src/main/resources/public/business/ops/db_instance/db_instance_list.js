...
function ListPage() {
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'backupType', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('备份类型'), templet:function (d){ return templet('backupType',fox.getDictText(SELECT_BACKUPTYPE_DATA,d.backupType,'','backupType'),d);}}
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					if(window.pageExt.list.selectBoxDataTransform) {
						opts.push(window.pageExt.list.selectBoxDataTransform("logMethod",{data:data[i],name:data[i].text,value:data[i].code},data[i],data,i));
					} else {
	...
	function bindButtonEvent() {
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'数据库实例'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'数据库实例'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			if (layEvent === 'edit') { // 修改
				...
				top.layer.confirm(fox.translate('确定删除此'+'数据库实例'+'吗？'), function (i) {
					top.layer.close(i);
