...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-ops/ops-auto-task-log-dtl";
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'ind', align:"right",fixed:false,  hide:false, sort: true  , title: fox.translate('序列') , templet: function (d) { return templet('ind',d.ind,d);}  }
					,{ field: 'content', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('记录') , templet: function (d) { return templet('content',d.content,d);}  }
	...
	function refreshTableData(sortField,sortType,reset) {
		...
		var value = {};
		value.ind={ inputType:"number_input", value: $("#ind").val() };
		var ps={searchField:"$composite"};
		if(window.pageExt.list.beforeQuery){
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('ops-auto-task-log-dtl-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('ops-auto-task-log-dtl-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'明细日志'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'明细日志'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('ops-auto-task-log-dtl-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				top.layer.load(2);
				top.layer.load(2);
				admin.post(getByIdURL, { id : data.id }, function (data) {
					...
					if(data.success) {
						admin.putTempData('ops-auto-task-log-dtl-form-data-form-action', "edit",true);
				...
				admin.post(getByIdURL, { id : data.id }, function (data) {
					top.layer.closeAll('loading');
					if(data.success) {
						admin.putTempData('ops-auto-task-log-dtl-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
				...
				top.layer.confirm(fox.translate('确定删除此'+'明细日志'+'吗？'), function (i) {
					top.layer.close(i);
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('ops-auto-task-log-dtl-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('ops-auto-task-log-dtl-form-data', data);
		var area=admin.getTempData('ops-auto-task-log-dtl-form-area');
		...
		admin.popupCenter({
			...
			area: ["80%",height+"px"],
			type: 2,
			id:"ops-auto-task-log-dtl-form-data-win",
			content: '/business/ops/auto_task_log_dtl/auto_task_log_dtl_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
