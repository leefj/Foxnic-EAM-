...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-ops/ops-db-recovery-rec";
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'recDate', align:"right", fixed:false, hide:false, sort: true   ,title: fox.translate('记录时间') ,templet: function (d) { return templet('recDate',fox.dateFormat(d.recDate,"yyyy-MM-dd"),d); }  }
	...
	function refreshTableData(sortField,sortType,reset) {
		...
		value.dbInstanceId={ inputType:"select_box", value: getSelectedValue("#dbInstanceId","value") ,fillBy:["dbInstance"]  , label:getSelectedValue("#dbInstanceId","nameStr") };
		value.type={ inputType:"radio_box", value: getSelectedValue("#type","value"), label:getSelectedValue("#type","nameStr") };
		value.recDate={ inputType:"date_input", begin: $("#recDate-begin").val(), end: $("#recDate-end").val() ,matchType:"auto" };
	...
	function initSearchFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.list.onSelectBoxChanged && window.pageExt.list.onSelectBoxChanged("dbInstanceId",data.arr,data.change,data.isAdd);
				},1);
			},
			paging: true,
			pageRemote: true,
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					if(window.pageExt.list.selectBoxDataTransform) {
						opts.push(window.pageExt.list.selectBoxDataTransform("dbInstanceId",{data:data[i],name:data[i].name,value:data[i].id},data[i],data,i));
					} else {
		...
		laydate.render({
			...
			trigger:"click",
				...
				setTimeout(function () {
					window.pageExt.list.onDatePickerChanged && window.pageExt.list.onDatePickerChanged("recDate",value, date, endDate);
				},1);
			}
		});
		laydate.render({
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('ops-db-recovery-rec-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('ops-db-recovery-rec-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'数据库恢复'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'数据库恢复'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			if (layEvent === 'edit') { // 修改
				...
				top.layer.confirm(fox.translate('确定删除此'+'数据库恢复'+'吗？'), function (i) {
					top.layer.close(i);
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('ops-db-recovery-rec-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('ops-db-recovery-rec-form-data', data);
		var area=admin.getTempData('ops-db-recovery-rec-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('数据库恢复');
		...
		admin.popupCenter({
			...
			area: ["80%",height+"px"],
			type: 2,
			id:"ops-db-recovery-rec-form-data-win",
			content: '/business/ops/db_recovery_rec/db_recovery_rec_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
