...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('ops-db-recovery-rec-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('ops-db-recovery-rec-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'ops-db-recovery-rec-form-data-win');
			if(area==null) return;
			admin.putTempData('ops-db-recovery-rec-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("dbInstanceId",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					if(window.pageExt.form.selectBoxDataTransform) {
						opts.push(window.pageExt.form.selectBoxDataTransform("dbInstanceId",{data:data[i],name:data[i].name,value:data[i].id,selected:(defaultValues.indexOf(data[i].id)!=-1 || defaultIndexs.indexOf(""+i)!=-1)},data[i],data,i));
					} else {
		...
		laydate.render({
			...
			trigger:"click",
				...
				window.pageExt.form.onDatePickerChanged && window.pageExt.form.onDatePickerChanged("recDate",value, date, endDate);
			}
		});
	    //渲染图片字段
		foxup.render({
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('ops-db-recovery-rec-form-data');
		}
		...
		if (hasData) {
			...
			//设置 记录时间 显示复选框勾选
			if(formData["recDate"]) {
				$("#recDate").val(fox.dateFormat(formData["recDate"],"yyyy-MM-dd"));
			}


			//设置  数据库 设置下拉框勾选
			fox.setSelectValue4QueryApi("#dbInstanceId",formData.dbInstance);
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('ops-db-recovery-rec-form-data');
		}
		return rawFormData;
	}
	...
	function getFormData() {
		...
		//获取 数据库 下拉框的值
		data["dbInstanceId"]=fox.getSelectedValue("dbInstanceId",false);
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('ops-db-recovery-rec-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('ops-db-recovery-rec-form-data-win',this); });
