...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('ops-security-info-lib-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('ops-security-info-lib-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'ops-security-info-lib-form-data-win');
			if(area==null) return;
			admin.putTempData('ops-security-info-lib-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			el: "secLevel",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("风险等级",'','cmp:form'),
			filterable: true,
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("secLevel",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			searchField: "label", //请自行调整用于搜索的字段名称
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					if(!data[i]) continue;
					if(window.pageExt.form.selectBoxDataTransform) {
						opts.push(window.pageExt.form.selectBoxDataTransform("secLevel",{data:data[i],name:data[i].label,value:data[i].code,selected:(defaultValues.indexOf(data[i].code)!=-1 || defaultIndexs.indexOf(""+i)!=-1)},data[i],data,i));
					} else {
						opts.push({data:data[i],name:data[i].label,value:data[i].code,selected:(defaultValues.indexOf(data[i].code)!=-1 || defaultIndexs.indexOf(""+i)!=-1)});
					}
				}
		...
		laydate.render({
			...
			trigger:"click",
				...
				window.pageExt.form.onDatePickerChanged && window.pageExt.form.onDatePickerChanged("businessData",value, date, endDate);
			}
		});
	}
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('ops-security-info-lib-form-data');
		}
		...
		if (hasData) {
			...
			if(formData["businessData"]) {
				$("#businessData").val(fox.dateFormat(formData["businessData"],"yyyy-MM-dd"));
			}
			...
			//设置  风险等级 设置下拉框勾选
			fox.setSelectValue4QueryApi("#secLevel",formData.levelDict);
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('ops-security-info-lib-form-data');
		}
		return rawFormData;
	}
	...
	function getFormData() {
		...
		//获取 风险等级 下拉框的值
		data["secLevel"]=fox.getSelectedValue("secLevel",false);
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('ops-security-info-lib-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('ops-security-info-lib-form-data-win',this); });
