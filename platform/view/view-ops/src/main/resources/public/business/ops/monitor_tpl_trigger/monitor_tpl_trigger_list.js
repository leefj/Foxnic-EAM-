...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-ops/ops-monitor-tpl-trigger";
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'ruleDiscovery', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('发现规则') , templet: function (d) { return templet('ruleDiscovery',d.ruleDiscovery,d);}  }
					,{ field: 'intervalTime', align:"right",fixed:false,  hide:false, sort: true  , title: fox.translate('间隔时间') , templet: function (d) { return templet('intervalTime',d.intervalTime,d);}  }
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('ops-monitor-tpl-trigger-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('ops-monitor-tpl-trigger-form-data-form-action', "create",true);
            showEditForm(data);
        };
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('ops-monitor-tpl-trigger-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				top.layer.load(2);
				top.layer.load(2);
				admin.post(getByIdURL, { id : data.id }, function (data) {
					...
					if(data.success) {
						admin.putTempData('ops-monitor-tpl-trigger-form-data-form-action', "edit",true);
				...
				admin.post(getByIdURL, { id : data.id }, function (data) {
					top.layer.closeAll('loading');
					if(data.success) {
						admin.putTempData('ops-monitor-tpl-trigger-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('ops-monitor-tpl-trigger-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('ops-monitor-tpl-trigger-form-data', data);
		var area=admin.getTempData('ops-monitor-tpl-trigger-form-area');
