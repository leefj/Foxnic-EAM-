...
function FormPage() {
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			...
			radio: false,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("脚本",'','cmp:form'),
			filterable: true,
			on: function(data){
				setTimeout(function () {
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					if(window.pageExt.form.selectBoxDataTransform) {
						opts.push(window.pageExt.form.selectBoxDataTransform("autoActionScriptIds",{data:data[i],name:data[i].name,value:data[i].id,selected:(defaultValues.indexOf(data[i].id)!=-1 || defaultIndexs.indexOf(""+i)!=-1)},data[i],data,i));
					} else {
		...
		fox.renderSelectBox({
			...
			radio: false,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("介质",'','cmp:form'),
			filterable: true,
			on: function(data){
				setTimeout(function () {
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					if(!data[i]) continue;
					if(window.pageExt.form.selectBoxDataTransform) {
						opts.push(window.pageExt.form.selectBoxDataTransform("autoActionFileIds",{data:data[i],name:data[i].name,value:data[i].id,selected:(defaultValues.indexOf(data[i].id)!=-1 || defaultIndexs.indexOf(""+i)!=-1)},data[i],data,i));
					} else {
						opts.push({data:data[i],name:data[i].name,value:data[i].id,selected:(defaultValues.indexOf(data[i].id)!=-1 || defaultIndexs.indexOf(""+i)!=-1)});
					}
				}
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('ops-auto-action-form-data');
		}
		return rawFormData;
	}
