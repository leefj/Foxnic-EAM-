...
function FormPage() {
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			el: "taskId",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("作业",'','cmp:form'),
			filterable: false,
			paging: true,
			pageRemote: true,
			on: function(data){
		...
		fox.renderSelectBox({
			el: "mLogId",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("日志",'','cmp:form'),
			filterable: false,
			paging: true,
			pageRemote: true,
			on: function(data){
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					if(!data[i]) continue;
					if(window.pageExt.form.selectBoxDataTransform) {
						opts.push(window.pageExt.form.selectBoxDataTransform("mLogId",{data:data[i],name:data[i].id,value:data[i].id,selected:(defaultValues.indexOf(data[i].id)!=-1 || defaultIndexs.indexOf(""+i)!=-1)},data[i],data,i));
					} else {
		...
		fox.renderSelectBox({
			el: "actionId",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("动作",'','cmp:form'),
			filterable: false,
			paging: true,
			pageRemote: true,
			on: function(data){
		...
		fox.renderSelectBox({
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					if(!data[i]) continue;
					if(window.pageExt.form.selectBoxDataTransform) {
						opts.push(window.pageExt.form.selectBoxDataTransform("nodeId",{data:data[i],name:data[i].name,value:data[i].id,selected:(defaultValues.indexOf(data[i].id)!=-1 || defaultIndexs.indexOf(""+i)!=-1)},data[i],data,i));
					} else {
						opts.push({data:data[i],name:data[i].name,value:data[i].id,selected:(defaultValues.indexOf(data[i].id)!=-1 || defaultIndexs.indexOf(""+i)!=-1)});
					}
				}
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('ops-auto-task-log-form-data');
		}
		return rawFormData;
	}
