...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-ops/ops-host-env-info";
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'ip', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('物理IP') , templet: function (d) { return templet('ip',d.ip,d);}  }
					,{ field: 'vip', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('VIP') , templet: function (d) { return templet('vip',d.vip,d);}  }
					,{ field: 'hostDesc', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('描述') , templet: function (d) { return templet('hostDesc',d.hostDesc,d);}  }
	...
	function refreshTableData(sortField,sortType,reset) {
		...
		value.vip={ inputType:"button",value: $("#vip").val() ,fuzzy: true,splitValue:false,valuePrefix:"",valueSuffix:"" };
		value.hostDesc={ inputType:"button",value: $("#hostDesc").val() ,fuzzy: true,splitValue:false,valuePrefix:"",valueSuffix:"" };
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('ops-host-env-info-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('ops-host-env-info-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'主机环境'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'主机环境'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('ops-host-env-info-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				top.layer.load(2);
				top.layer.load(2);
				admin.post(getByIdURL, { id : data.id }, function (data) {
					...
					if(data.success) {
						admin.putTempData('ops-host-env-info-form-data-form-action', "edit",true);
				...
				admin.post(getByIdURL, { id : data.id }, function (data) {
					top.layer.closeAll('loading');
					if(data.success) {
						admin.putTempData('ops-host-env-info-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
				...
				top.layer.confirm(fox.translate('确定删除此'+'主机环境'+'吗？'), function (i) {
					top.layer.close(i);
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('ops-host-env-info-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('ops-host-env-info-form-data', data);
		var area=admin.getTempData('ops-host-env-info-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('主机环境');
		...
		admin.popupCenter({
			...
			area: ["95%",height+"px"],
			type: 2,
			id:"ops-host-env-info-form-data-win",
			content: '/business/ops/host_env_info/host_env_info_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
