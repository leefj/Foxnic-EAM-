...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('hr-indicator-lib-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('hr-indicator-lib-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'hr-indicator-lib-form-data-win');
			if(area==null) return;
			admin.putTempData('hr-indicator-lib-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			el: "typeCode",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("指标分类",'','cmp:form'),
			filterable: false,
			layVerify: 'required',
			layVerType: 'msg',
			on: function(data){
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					if(window.pageExt.form.selectBoxDataTransform) {
						opts.push(window.pageExt.form.selectBoxDataTransform("typeCode",{data:data[i],name:data[i].label,value:data[i].code,selected:(defaultValues.indexOf(data[i].code)!=-1 || defaultIndexs.indexOf(""+i)!=-1)},data[i],data,i));
					} else {
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('hr-indicator-lib-form-data');
		}
		...
		if (hasData) {
			...
			//设置  指标分类 设置下拉框勾选
			fox.setSelectValue4QueryApi("#typeCode",formData.typeDict);
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('hr-indicator-lib-form-data');
		}
		return rawFormData;
	}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('hr-indicator-lib-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('hr-indicator-lib-form-data-win',this); });
