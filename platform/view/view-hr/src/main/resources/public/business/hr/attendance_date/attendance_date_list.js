...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-hr/hr-attendance-date";
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'year', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('年份') , templet: function (d) { return templet('year',d.year,d);}  }
					,{ field: 'dateStr', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('日期') , templet: function (d) { return templet('dateStr',d.dateStr,d);}  }
					,{ field: 'week', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('周'), templet:function (d){ return templet('week',fox.getEnumText(RADIO_WEEK_DATA,d.week,'','week'),d);}}
	...
	function refreshTableData(sortField,sortType,reset) {
		...
		var value = {};
		value.year={ inputType:"button",value: $("#year").val()};
		value.dateStr={ inputType:"button",value: $("#dateStr").val() ,fuzzy: true,splitValue:false,valuePrefix:"",valueSuffix:"" };
		value.week={ inputType:"radio_box", value: getSelectedValue("#week","value"), label:getSelectedValue("#week","nameStr") };
		value.statutoryHoliday={ inputType:"radio_box", value: getSelectedValue("#statutoryHoliday","value"), label:getSelectedValue("#statutoryHoliday","nameStr") };
		value.holiday={ inputType:"button",value: $("#holiday").val()};
		value.attendancSign={ inputType:"radio_box", value: getSelectedValue("#attendancSign","value"), label:getSelectedValue("#attendancSign","nameStr") };
		var ps={searchField:"$composite"};
		if(window.pageExt.list.beforeQuery){
	...
	function initSearchFields() {
			...
			transform:function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					if(window.pageExt.list.selectBoxDataTransform) {
						opts.push(window.pageExt.list.selectBoxDataTransform("week",{data:data[i],name:data[i].text,value:data[i].code},data[i],data,i));
					} else {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.list.onSelectBoxChanged && window.pageExt.list.onSelectBoxChanged("statutoryHoliday",data.arr,data.change,data.isAdd);
				},1);
			},
			//toolbar: {show:true,showIcon:true,list:["CLEAR","REVERSE"]},
			transform:function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					if(window.pageExt.list.selectBoxDataTransform) {
						opts.push(window.pageExt.list.selectBoxDataTransform("statutoryHoliday",{data:data[i],name:data[i].text,value:data[i].code},data[i],data,i));
					} else {
						opts.push({data:data[i],name:data[i].text,value:data[i].code});
					}
				}
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.list.onSelectBoxChanged && window.pageExt.list.onSelectBoxChanged("attendancSign",data.arr,data.change,data.isAdd);
				},1);
			},
			//toolbar: {show:true,showIcon:true,list:["CLEAR","REVERSE"]},
			transform:function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					if(window.pageExt.list.selectBoxDataTransform) {
						opts.push(window.pageExt.list.selectBoxDataTransform("attendancSign",{data:data[i],name:data[i].text,value:data[i].code},data[i],data,i));
					} else {
						opts.push({data:data[i],name:data[i].text,value:data[i].code});
					}
				}
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('hr-attendance-date-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('hr-attendance-date-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'考勤日期'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'考勤日期'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('hr-attendance-date-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				top.layer.load(2);
				top.layer.load(2);
				admin.post(getByIdURL, { id : data.id }, function (data) {
					...
					if(data.success) {
						admin.putTempData('hr-attendance-date-form-data-form-action', "edit",true);
				...
				admin.post(getByIdURL, { id : data.id }, function (data) {
					top.layer.closeAll('loading');
					if(data.success) {
						admin.putTempData('hr-attendance-date-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
				...
				top.layer.confirm(fox.translate('确定删除此'+'考勤日期'+'吗？'), function (i) {
					top.layer.close(i);
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('hr-attendance-date-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('hr-attendance-date-form-data', data);
		var area=admin.getTempData('hr-attendance-date-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('考勤日期');
		...
		admin.popupCenter({
			...
			content: '/business/hr/attendance_date/attendance_date_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
