...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('hr-salary-tax-pct-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('hr-salary-tax-pct-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'hr-salary-tax-pct-form-data-win');
			if(area==null) return;
			admin.putTempData('hr-salary-tax-pct-form-area', area);
	...
	function renderFormFields() {
		...
		form.on('radio(minInclude)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=minInclude]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("minInclude",data,checked);
		});
		form.on('radio(maxInclude)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=maxInclude]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("maxInclude",data,checked);
		});
	}
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('hr-salary-tax-pct-form-data');
		}
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('hr-salary-tax-pct-form-data');
		}
		return rawFormData;
	}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('hr-salary-tax-pct-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('hr-salary-tax-pct-form-data-win',this); });
