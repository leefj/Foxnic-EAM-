...
function FormPage() {
	...
	// 接口地址
	const moduleURL="/service-hr/hr-assessment-bill-task-dtl";
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('hr-assessment-bill-task-dtl-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('hr-assessment-bill-task-dtl-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'hr-assessment-bill-task-dtl-form-data-win');
			if(area==null) return;
			admin.putTempData('hr-assessment-bill-task-dtl-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			el: "billTaskId",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("单据",'','cmp:form'),
			filterable: true,
			paging: true,
			pageRemote: true,
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("billTaskId",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					if(window.pageExt.form.selectBoxDataTransform) {
						opts.push(window.pageExt.form.selectBoxDataTransform("billTaskId",{data:data[i],name:data[i].name,value:data[i].id,selected:(defaultValues.indexOf(data[i].id)!=-1 || defaultIndexs.indexOf(""+i)!=-1)},data[i],data,i));
					} else {
		...
		form.on('radio(relationship)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=relationship]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("relationship",data,checked);
		});
		laydate.render({
			elem: '#rcdTime',
			type:"date",
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('hr-assessment-bill-task-dtl-form-data');
		}
		...
		if (hasData) {
			...
			//设置  单据 设置下拉框勾选
			fox.setSelectValue4QueryApi("#billTaskId",formData.AssessmentBillTask);
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('hr-assessment-bill-task-dtl-form-data');
		}
		return rawFormData;
	}
	...
	function getFormData() {
		...
		//获取 单据 下拉框的值
		data["billTaskId"]=fox.getSelectedValue("billTaskId",false);
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('hr-assessment-bill-task-dtl-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('hr-assessment-bill-task-dtl-form-data-win',this); });
