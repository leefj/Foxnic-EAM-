...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-hr/hr-person-work-experience";
	...
	function refreshTableData(sortField,sortType,reset) {
		...
		value.post={ inputType:"button",value: $("#post").val()};
		value.workContent={ inputType:"button",value: $("#workContent").val()};
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'approval':
					...
					break;
				case 'create':
					admin.putTempData('hr-person-work-experience-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
		...
		function openApprovalFrom() {
			//设置新增是初始化数据
			var data={};
			admin.putTempData('hr-person-work-experience-form-data-form-action', "create",true);
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'工作经历'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'工作经历'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('hr-person-work-experience-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				top.layer.load(2);
				top.layer.load(2);
				admin.post(getByIdURL, { id : data.id }, function (data) {
					...
					if(data.success) {
						admin.putTempData('hr-person-work-experience-form-data-form-action', "edit",true);
				...
				admin.post(getByIdURL, { id : data.id }, function (data) {
					top.layer.closeAll('loading');
					if(data.success) {
						admin.putTempData('hr-person-work-experience-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
				...
				top.layer.confirm(fox.translate('确定删除此'+'工作经历'+'吗？'), function (i) {
					top.layer.close(i);
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('hr-person-work-experience-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('hr-person-work-experience-form-data', data);
		var area=admin.getTempData('hr-person-work-experience-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('工作经历');
		...
		admin.popupCenter({
			...
			area: ["65%",height+"px"],
			type: 2,
			id:"hr-person-work-experience-form-data-win",
			content: '/business/hr/person_work_experience/person_work_a_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
