...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-hr/hr-attendance-data-process";
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'kqRq', align:"right", fixed:false, hide:false, sort: true   ,title: fox.translate('考勤日期') ,templet: function (d) { return templet('kqRq',fox.dateFormat(d.kqRq,"yyyy-MM-dd"),d); }  }
					,{ field: 'processDate', align:"right", fixed:false, hide:false, sort: true   ,title: fox.translate('处理时间') ,templet: function (d) { return templet('processDate',fox.dateFormat(d.processDate,"yyyy-MM-dd HH:mm:ss"),d); }  }
	...
	function refreshTableData(sortField,sortType,reset) {
		...
		value.kqRq={ inputType:"date_input", begin: $("#kqRq-begin").val(), end: $("#kqRq-end").val() ,matchType:"auto" };
		var ps={searchField:"$composite"};
		if(window.pageExt.list.beforeQuery){
	...
	function initSearchFields() {
		...
		laydate.render({
			...
			trigger:"click",
				...
				setTimeout(function () {
					window.pageExt.list.onDatePickerChanged && window.pageExt.list.onDatePickerChanged("kqRq",value, date, endDate);
				},1);
			}
		});
		laydate.render({
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('hr-attendance-data-process-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('hr-attendance-data-process-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'处理记录'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'处理记录'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('hr-attendance-data-process-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				top.layer.load(2);
				top.layer.load(2);
				admin.post(getByIdURL, { id : data.id }, function (data) {
					...
					if(data.success) {
						admin.putTempData('hr-attendance-data-process-form-data-form-action', "edit",true);
				...
				admin.post(getByIdURL, { id : data.id }, function (data) {
					top.layer.closeAll('loading');
					if(data.success) {
						admin.putTempData('hr-attendance-data-process-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
				...
				top.layer.confirm(fox.translate('确定删除此'+'处理记录'+'吗？'), function (i) {
					top.layer.close(i);
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('hr-attendance-data-process-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('hr-attendance-data-process-form-data', data);
		var area=admin.getTempData('hr-attendance-data-process-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('处理记录');
		...
		admin.popupCenter({
			...
			area: ["75%",height+"px"],
			type: 2,
			id:"hr-attendance-data-process-form-data-win",
			content: '/business/hr/attendance_data_process/attendance_data_process_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
