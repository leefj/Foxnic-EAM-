...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-hr/hr-person-education";
	...
	function refreshTableData(sortField,sortType,reset) {
		...
		value.education={ inputType:"select_box", value: getSelectedValue("#education","value") ,fillBy:["educationInfo"]  , label:getSelectedValue("#education","nameStr") };
		value.sTime={ inputType:"date_input", value: $("#sTime").val() ,matchType:"auto"};
		value.eTime={ inputType:"date_input", value: $("#eTime").val() ,matchType:"auto"};
		value.educationalInstitutions={ inputType:"button",value: $("#educationalInstitutions").val()};
		value.majoyStudied={ inputType:"button",value: $("#majoyStudied").val()};
	...
	function initSearchFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.list.onSelectBoxChanged && window.pageExt.list.onSelectBoxChanged("education",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					if(!data[i]) continue;
					if(window.pageExt.list.selectBoxDataTransform) {
						opts.push(window.pageExt.list.selectBoxDataTransform("education",{data:data[i],name:data[i].label,value:data[i].code},data[i],data,i));
					} else {
		...
		laydate.render({
			...
			trigger:"click",
				...
				setTimeout(function () {
					window.pageExt.list.onDatePickerChanged && window.pageExt.list.onDatePickerChanged("eTime",value, date, endDate);
				},1);
			}
		});
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('hr-person-education-form-data', {});
					openCreateFrom();
					break;
				case 'approval':
		...
		function openApprovalFrom() {
			//设置新增是初始化数据
			var data={};
			admin.putTempData('hr-person-education-form-data-form-action', "create",true);
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'教育信息'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'教育信息'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('hr-person-education-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				top.layer.load(2);
				top.layer.load(2);
				admin.post(getByIdURL, { id : data.id }, function (data) {
					...
					if(data.success) {
						admin.putTempData('hr-person-education-form-data-form-action', "edit",true);
				...
				admin.post(getByIdURL, { id : data.id }, function (data) {
					top.layer.closeAll('loading');
					if(data.success) {
						admin.putTempData('hr-person-education-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
				...
				top.layer.confirm(fox.translate('确定删除此'+'教育信息'+'吗？'), function (i) {
					top.layer.close(i);
	...
	function showApprovalEditForm(data) {
		...
		var action=admin.getTempData('hr-person-education-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('hr-person-education-form-data', data);
		var area=admin.getTempData('hr-person-education-form-area');
		...
		admin.popupCenter({
			...
			area: ["80%",height+"px"],
			type: 2,
			id:"hr-person-education-form-data-win",
	...
	function showEditForm(data) {
		...
		admin.popupCenter({
			...
			type: 2,
			id:"hr-person-education-form-data-win",
			content: '/business/hr/person_education/person_edu_a_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
