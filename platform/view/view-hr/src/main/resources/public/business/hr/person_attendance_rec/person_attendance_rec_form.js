...
function FormPage() {
	...
	// 接口地址
	const moduleURL="/service-hr/hr-person-attendance-rec";
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('hr-person-attendance-rec-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('hr-person-attendance-rec-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'hr-person-attendance-rec-form-data-win');
			if(area==null) return;
			admin.putTempData('hr-person-attendance-rec-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			el: "employeeId",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("员工",'','cmp:form'),
			filterable: true,
			paging: true,
			pageRemote: true,
			on: function(data){
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					if(window.pageExt.form.selectBoxDataTransform) {
						opts.push(window.pageExt.form.selectBoxDataTransform("employeeId",{data:data[i],name:data[i].name,value:data[i].id,selected:(defaultValues.indexOf(data[i].id)!=-1 || defaultIndexs.indexOf(""+i)!=-1)},data[i],data,i));
					} else {
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('hr-person-attendance-rec-form-data');
		}
		...
		if (hasData) {
			...
			//设置 记录时间 显示复选框勾选
			if(formData["recTime"]) {
				$("#recTime").val(fox.dateFormat(formData["recTime"],"yyyy-MM-dd"));
			}
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('hr-person-attendance-rec-form-data');
		}
		return rawFormData;
	}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('hr-person-attendance-rec-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('hr-person-attendance-rec-form-data-win',this); });
