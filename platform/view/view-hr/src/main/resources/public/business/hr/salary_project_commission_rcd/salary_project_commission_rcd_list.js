...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-hr/hr-salary-project-commission-rcd";
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'projectCommissionRatio', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('提成比例') , templet: function (d) { return templet('projectCommissionRatio',d.projectCommissionRatio,d);}  }
					,{ field: 'businessValue', align:"right",fixed:false,  hide:false, sort: true  , title: fox.translate('业务量') , templet: function (d) { return templet('businessValue',d.businessValue,d);}  }
					,{ field: 'commissionSalary', align:"right",fixed:false,  hide:false, sort: true  , title: fox.translate('提成金额') , templet: function (d) { return templet('commissionSalary',d.commissionSalary,d);}  }
					,{ field: 'rcdDate', align:"right", fixed:false, hide:false, sort: true   ,title: fox.translate('记录日期') ,templet: function (d) { return templet('rcdDate',fox.dateFormat(d.rcdDate,"yyyy-MM-dd"),d); }  }
					...
					,{ field: 'projectCodeValue', align:"",fixed:false,  hide:false, sort: false  , title: fox.translate('项目编码') , templet: function (d) { return templet('projectCodeValue',fox.getProperty(d,["project","code"],0,'','projectCodeValue'),d);} }
	...
	function refreshTableData(sortField,sortType,reset) {
		...
		value.projectCode={ inputType:"select_box", value: $("#projectCode").val() ,fuzzy: true,splitValue:false,valuePrefix:"",valueSuffix:"" ,fillBy:["project"] };
		value.projectName={ inputType:"button",value: $("#projectName").val() ,fuzzy: true,splitValue:false,valuePrefix:"",valueSuffix:"" };
		value.rcdDate={ inputType:"date_input", begin: $("#rcdDate-begin").val(), end: $("#rcdDate-end").val() ,matchType:"auto" };
	...
	function initSearchFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.list.onSelectBoxChanged && window.pageExt.list.onSelectBoxChanged("projectCode",data.arr,data.change,data.isAdd);
				},1);
			},
			paging: true,
			pageRemote: true,
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					if(!data[i]) continue;
					if(window.pageExt.list.selectBoxDataTransform) {
						opts.push(window.pageExt.list.selectBoxDataTransform("projectCode",{data:data[i],name:data[i].name,value:data[i].code},data[i],data,i));
					} else {
		...
		laydate.render({
			...
			trigger:"click",
				...
				setTimeout(function () {
					window.pageExt.list.onDatePickerChanged && window.pageExt.list.onDatePickerChanged("rcdDate",value, date, endDate);
				},1);
			}
		});
		laydate.render({
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('hr-salary-project-commission-rcd-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('hr-salary-project-commission-rcd-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'项目提成记录'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'项目提成记录'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('hr-salary-project-commission-rcd-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				top.layer.load(2);
				top.layer.load(2);
				admin.post(getByIdURL, { id : data.id }, function (data) {
					...
					if(data.success) {
						admin.putTempData('hr-salary-project-commission-rcd-form-data-form-action', "edit",true);
				...
				admin.post(getByIdURL, { id : data.id }, function (data) {
					top.layer.closeAll('loading');
					if(data.success) {
						admin.putTempData('hr-salary-project-commission-rcd-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
				...
				top.layer.confirm(fox.translate('确定删除此'+'项目提成记录'+'吗？'), function (i) {
					top.layer.close(i);
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('hr-salary-project-commission-rcd-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('hr-salary-project-commission-rcd-form-data', data);
		var area=admin.getTempData('hr-salary-project-commission-rcd-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('项目提成记录');
		...
		admin.popupCenter({
			...
			area: ["80%",height+"px"],
			type: 2,
			id:"hr-salary-project-commission-rcd-form-data-win",
			content: '/business/hr/salary_project_commission_rcd/salary_project_commission_rcd_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
