...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('hr-attendance-data-process-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('hr-attendance-data-process-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'hr-attendance-data-process-form-data-win');
			if(area==null) return;
			admin.putTempData('hr-attendance-data-process-form-area', area);
	...
	function renderFormFields() {
		...
		laydate.render({
			...
			trigger:"click",
				...
				window.pageExt.form.onDatePickerChanged && window.pageExt.form.onDatePickerChanged("kqRq",value, date, endDate);
			}
		});
		laydate.render({
			elem: '#processDate',
			...
			trigger:"click",
				...
				window.pageExt.form.onDatePickerChanged && window.pageExt.form.onDatePickerChanged("processDate",value, date, endDate);
			}
		});
	}
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('hr-attendance-data-process-form-data');
		}
		...
		if (hasData) {
			...
			//设置 考勤日期 显示复选框勾选
			if(formData["kqRq"]) {
				$("#kqRq").val(fox.dateFormat(formData["kqRq"],"yyyy-MM-dd"));
			}
			//设置 处理时间 显示复选框勾选
			if(formData["processDate"]) {
				$("#processDate").val(fox.dateFormat(formData["processDate"],"yyyy-MM-dd HH:mm:ss"));
			}
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('hr-attendance-data-process-form-data');
		}
		return rawFormData;
	}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('hr-attendance-data-process-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('hr-attendance-data-process-form-data-win',this); });
