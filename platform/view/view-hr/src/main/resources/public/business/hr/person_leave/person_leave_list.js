...
function ListPage() {
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('hr-person-leave-form-data', {});
					var defaultValue={};
					if(window.pageExt.list.getBpmViewConfig) {
						defaultValue=window.pageExt.list.getBpmViewConfig(obj.event);
					}
					admin.putTempData('hr-person-leave-form-data-form-action', "create",true);
					bpm.openProcessView(null,null,false,{"formDefinitionCode":"hr_person_leave"},refreshTableData,refreshRowData,"bill",defaultValue);
					break;
				case 'batch-del':
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('hr-person-leave-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				top.layer.load(2);
				bpm.getProcessInstanceByBill("hr_person_leave",{ id : data.id },function(p) {
					top.layer.closeAll('loading');
					if(p) {
						admin.putTempData('hr-person-leave-form-data-form-action', "edit",true);
						bpm.openProcessView(p.id,null,false,{"formDefinitionCode":"hr_person_leave"},refreshTableData,refreshRowData,"bill");
					} else {
				...
				admin.post(getByIdURL, { id : data.id }, function (data) {
					top.layer.closeAll('loading');
					if(data.success) {
						admin.putTempData('hr-person-leave-form-data-form-action', "view",true);
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('hr-person-leave-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('hr-person-leave-form-data', data);
		var area=admin.getTempData('hr-person-leave-form-area');
		...
		admin.popupCenter({
			...
			area: ["75%",height+"px"],
			type: 2,
			id:"hr-person-leave-form-data-win",
			content: '/business/hr/person_leave/person_leave_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
