...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-hr/hr-assessment-plan";
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('hr-assessment-plan-form-data', {});
					var defaultValue={};
					if(window.pageExt.list.getBpmViewConfig) {
						defaultValue=window.pageExt.list.getBpmViewConfig(obj.event);
					}
					admin.putTempData('hr-assessment-plan-form-data-form-action', "create",true);
					bpm.openProcessView(null,null,false,{"formDefinitionCode":"hr_assessment_plan"},refreshTableData,refreshRowData,"bill",defaultValue);
					break;
				case 'batch-del':
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'考核计划'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'考核计划'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('hr-assessment-plan-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				top.layer.load(2);
				bpm.getProcessInstanceByBill("hr_assessment_plan",{ id : data.id },function(p) {
					top.layer.closeAll('loading');
					if(p) {
						admin.putTempData('hr-assessment-plan-form-data-form-action', "edit",true);
						bpm.openProcessView(p.id,null,false,{"formDefinitionCode":"hr_assessment_plan"},refreshTableData,refreshRowData,"bill");
					} else {
				...
				admin.post(getByIdURL, { id : data.id }, function (data) {
					top.layer.closeAll('loading');
					if(data.success) {
						admin.putTempData('hr-assessment-plan-form-data-form-action', "view",true);
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('hr-assessment-plan-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('hr-assessment-plan-form-data', data);
		var area=admin.getTempData('hr-assessment-plan-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('考核计划');
		...
		admin.popupCenter({
			...
			area: ["80%",height+"px"],
			type: 2,
			id:"hr-assessment-plan-form-data-win",
			content: '/business/hr/assessment_plan/assessment_plan_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
