...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-hr/hr-assessment-cycle";
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'name', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('考核主题') , templet: function (d) { return templet('name',d.name,d);}  }
					,{ field: 'info', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('考核说明') , templet: function (d) { return templet('info',d.info,d);}  }
					...
					,{ field: 'sdate', align:"right", fixed:false, hide:false, sort: true   ,title: fox.translate('开始时间') ,templet: function (d) { return templet('sdate',fox.dateFormat(d.sdate,"yyyy-MM-dd HH:mm:ss"),d); }  }
					,{ field: 'edate', align:"right", fixed:false, hide:false, sort: true   ,title: fox.translate('结束时间') ,templet: function (d) { return templet('edate',fox.dateFormat(d.edate,"yyyy-MM-dd HH:mm:ss"),d); }  }
					...
					,{ field: 'rcdUserId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('提及人') , templet: function (d) { return templet('rcdUserId',d.rcdUserId,d);}  }
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('hr-assessment-cycle-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('hr-assessment-cycle-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'考核周期'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'考核周期'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('hr-assessment-cycle-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				top.layer.load(2);
				top.layer.load(2);
				admin.post(getByIdURL, { id : data.id }, function (data) {
					...
					if(data.success) {
						admin.putTempData('hr-assessment-cycle-form-data-form-action', "edit",true);
				...
				admin.post(getByIdURL, { id : data.id }, function (data) {
					top.layer.closeAll('loading');
					if(data.success) {
						admin.putTempData('hr-assessment-cycle-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
				...
				top.layer.confirm(fox.translate('确定删除此'+'考核周期'+'吗？'), function (i) {
					top.layer.close(i);
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('hr-assessment-cycle-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('hr-assessment-cycle-form-data', data);
		var area=admin.getTempData('hr-assessment-cycle-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('考核周期');
		...
		admin.popupCenter({
			...
			area: ["75%",height+"px"],
			type: 2,
			id:"hr-assessment-cycle-form-data-win",
			content: '/business/hr/assessment_cycle/assessment_cycle_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
