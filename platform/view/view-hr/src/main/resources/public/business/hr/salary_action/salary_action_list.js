...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-hr/hr-salary-action";
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'start', align:"right", fixed:false, hide:false, sort: true   ,title: fox.translate('开始日期') ,templet: function (d) { return templet('start',fox.dateFormat(d.start,"yyyy-MM-dd"),d); }  }
	...
	function refreshTableData(sortField,sortType,reset) {
		...
		value.actionMonth={ inputType:"select_box", value: $("#actionMonth").val() ,fuzzy: true,splitValue:false,valuePrefix:"",valueSuffix:"" ,fillBy:["salaryMonth"] };
		value.tplId={ inputType:"select_box", value: getSelectedValue("#tplId","value") ,fillBy:["salaryTpl"]  , label:getSelectedValue("#tplId","nameStr") };
		var ps={searchField:"$composite"};
		if(window.pageExt.list.beforeQuery){
	...
	function initSearchFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.list.onSelectBoxChanged && window.pageExt.list.onSelectBoxChanged("actionMonth",data.arr,data.change,data.isAdd);
				},1);
			},
			paging: true,
			pageRemote: true,
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					if(!data[i]) continue;
					if(window.pageExt.list.selectBoxDataTransform) {
						opts.push(window.pageExt.list.selectBoxDataTransform("actionMonth",{data:data[i],name:data[i].name,value:data[i].code},data[i],data,i));
					} else {
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('hr-salary-action-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('hr-salary-action-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'薪酬发放'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'薪酬发放'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			if (layEvent === 'edit') { // 修改
				...
				top.layer.confirm(fox.translate('确定删除此'+'薪酬发放'+'吗？'), function (i) {
					top.layer.close(i);
				...
				window.pageExt.list.salaryDetail(data,this);
			}
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('hr-salary-action-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('hr-salary-action-form-data', data);
		var area=admin.getTempData('hr-salary-action-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('薪酬发放');
		...
		admin.popupCenter({
			...
			area: ["65%",height+"px"],
			type: 2,
			id:"hr-salary-action-form-data-win",
			content: '/business/hr/salary_action/salary_action_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
