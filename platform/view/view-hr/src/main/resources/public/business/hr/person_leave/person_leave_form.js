...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('hr-person-leave-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('hr-person-leave-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'hr-person-leave-form-data-win');
			if(area==null) return;
			admin.putTempData('hr-person-leave-form-area', area);
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('hr-person-leave-form-data');
		}
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('hr-person-leave-form-data');
		}
		return rawFormData;
	}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('hr-person-leave-form-data-win');
				}
    ...
    function bindButtonEvent() {
		...
		// 请选择人员对话框
		$("#handoverId-button").click(function(){
				var handoverIdDialogOptions={
				field:"handoverId",
				formData:getFormData(),
				inputEl:$("#handoverId"),
				buttonEl:$(this),
			...
			fox.chooseEmployee(handoverIdDialogOptions);
		});

	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('hr-person-leave-form-data-win',this); });
