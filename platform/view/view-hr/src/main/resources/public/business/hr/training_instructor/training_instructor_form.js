...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('hr-training-instructor-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('hr-training-instructor-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'hr-training-instructor-form-data-win');
			if(area==null) return;
			admin.putTempData('hr-training-instructor-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			el: "orgId",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("工作单位",'','cmp:form'),
			filterable: true,
			paging: true,
			pageRemote: true,
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("orgId",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					if(window.pageExt.form.selectBoxDataTransform) {
						opts.push(window.pageExt.form.selectBoxDataTransform("orgId",{data:data[i],name:data[i].name,value:data[i].id,selected:(defaultValues.indexOf(data[i].id)!=-1 || defaultIndexs.indexOf(""+i)!=-1)},data[i],data,i));
					} else {
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('hr-training-instructor-form-data');
		}
		...
		if (hasData) {
			...
			//设置  工作单位 设置下拉框勾选
			fox.setSelectValue4QueryApi("#orgId",formData.trainingInstitution);
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('hr-training-instructor-form-data');
		}
		return rawFormData;
	}
	...
	function getFormData() {
		...
		//获取 工作单位 下拉框的值
		data["orgId"]=fox.getSelectedValue("orgId",false);
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('hr-training-instructor-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('hr-training-instructor-form-data-win',this); });
