...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('hr-assessment-bill-task-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('hr-assessment-bill-task-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'hr-assessment-bill-task-form-data-win');
			if(area==null) return;
			admin.putTempData('hr-assessment-bill-task-form-area', area);
	...
	function renderFormFields() {
		...
		laydate.render({
			...
			trigger:"click",
				...
				window.pageExt.form.onDatePickerChanged && window.pageExt.form.onDatePickerChanged("stimeStr",value, date, endDate);
			}
		});
		laydate.render({
			elem: '#etimeStr',
			...
			trigger:"click",
				...
				window.pageExt.form.onDatePickerChanged && window.pageExt.form.onDatePickerChanged("etimeStr",value, date, endDate);
			}
		});
	}
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('hr-assessment-bill-task-form-data');
		}
		...
		if (hasData) {
			...
			//设置 开始时间 显示复选框勾选
			if(formData["stimeStr"]) {
				$("#stimeStr").val(fox.dateFormat(formData["stimeStr"],"yyyy-MM-dd HH:mm:ss"));
			}
			//设置 结束时间 显示复选框勾选
			if(formData["etimeStr"]) {
				$("#etimeStr").val(fox.dateFormat(formData["etimeStr"],"yyyy-MM-dd HH:mm:ss"));
			}
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('hr-assessment-bill-task-form-data');
		}
		return rawFormData;
	}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('hr-assessment-bill-task-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('hr-assessment-bill-task-form-data-win',this); });
