...
function ListPage() {
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'personId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('姓名'), templet: function (d) { return templet('personId' ,fox.joinLabel(d.person,"name",',','','personId'),d);}}
					,{ field: 'typeCode', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('类型'), templet: function (d) { return templet('typeCode' ,fox.joinLabel(d.personBusiInsureType,"name",',','','typeCode'),d);}}
					,{ field: 'billCode', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('保险单号') , templet: function (d) { return templet('billCode',d.billCode,d);}  }
					,{ field: 'rcdTime', align:"right", fixed:false, hide:false, sort: true   ,title: fox.translate('登记时间') ,templet: function (d) { return templet('rcdTime',fox.dateFormat(d.rcdTime,"yyyy-MM-dd"),d); }  }
					,{ field: 'pay', align:"right",fixed:false,  hide:false, sort: true  , title: fox.translate('保险金额') , templet: function (d) { return templet('pay',d.pay,d);}  }
					,{ field: 'personPay', align:"right",fixed:false,  hide:false, sort: true  , title: fox.translate('个人支付') , templet: function (d) { return templet('personPay',d.personPay,d);}  }
					,{ field: 'companyPay', align:"right",fixed:false,  hide:false, sort: true  , title: fox.translate('公司支付') , templet: function (d) { return templet('companyPay',d.companyPay,d);}  }
					,{ field: 'startTime', align:"right", fixed:false, hide:false, sort: true   ,title: fox.translate('生效时间') ,templet: function (d) { return templet('startTime',fox.dateFormat(d.startTime,"yyyy-MM-dd"),d); }  }
					,{ field: 'endTime', align:"right", fixed:false, hide:false, sort: true   ,title: fox.translate('失效时间') ,templet: function (d) { return templet('endTime',fox.dateFormat(d.endTime,"yyyy-MM-dd"),d); }  }
					...
					,{ field: 'personCardNumber', align:"",fixed:false,  hide:false, sort: false  , title: fox.translate('身份证') , templet: function (d) { return templet('personCardNumber',fox.getProperty(d,["person","identityCard"],0,'','personCardNumber'),d);} }
	...
	function refreshTableData(sortField,sortType,reset) {
		...
		value.typeCode={ inputType:"select_box", value: getSelectedValue("#typeCode","value") ,fillBy:["personBusiInsureType"]  , label:getSelectedValue("#typeCode","nameStr") };
	...
	function initSearchFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.list.onSelectBoxChanged && window.pageExt.list.onSelectBoxChanged("typeCode",data.arr,data.change,data.isAdd);
				},1);
			},
			paging: true,
			pageRemote: true,
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					if(!data[i]) continue;
					if(window.pageExt.list.selectBoxDataTransform) {
						opts.push(window.pageExt.list.selectBoxDataTransform("typeCode",{data:data[i],name:data[i].name,value:data[i].code},data[i],data,i));
					} else {
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('hr-person-busi-insure-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('hr-person-busi-insure-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'商业保险'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'商业保险'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('hr-person-busi-insure-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				top.layer.load(2);
				top.layer.load(2);
				admin.post(getByIdURL, { id : data.id }, function (data) {
					...
					if(data.success) {
						admin.putTempData('hr-person-busi-insure-form-data-form-action', "edit",true);
				...
				admin.post(getByIdURL, { id : data.id }, function (data) {
					top.layer.closeAll('loading');
					if(data.success) {
						admin.putTempData('hr-person-busi-insure-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
				...
				top.layer.confirm(fox.translate('确定删除此'+'商业保险'+'吗？'), function (i) {
					top.layer.close(i);
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('hr-person-busi-insure-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('hr-person-busi-insure-form-data', data);
		var area=admin.getTempData('hr-person-busi-insure-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('商业保险');
		...
		admin.popupCenter({
			...
			area: ["75%",height+"px"],
			type: 2,
			id:"hr-person-busi-insure-form-data-win",
			content: '/business/hr/person_busi_insure/person_busi_insure_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
