...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-hr/hr-person-certificate";
	...
	function refreshTableData(sortField,sortType,reset) {
		...
		value.getTime={ inputType:"date_input", value: $("#getTime").val() ,matchType:"auto"};
	...
	function initSearchFields() {
		...
		laydate.render({
			...
			trigger:"click",
				...
				setTimeout(function () {
					window.pageExt.list.onDatePickerChanged && window.pageExt.list.onDatePickerChanged("getTime",value, date, endDate);
				},1);
			}
		});
	...
	function bindButtonEvent() {
		...
		function openApprovalFrom() {
			//设置新增是初始化数据
			var data={};
			admin.putTempData('hr-person-certificate-form-data-form-action', "create",true);
			showEditApprovalForm(data);
		};
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'证书信息'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'证书信息'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('hr-person-certificate-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				top.layer.load(2);
				top.layer.load(2);
				admin.post(getByIdURL, { id : data.id }, function (data) {
					...
					if(data.success) {
						admin.putTempData('hr-person-certificate-form-data-form-action', "edit",true);
				...
				admin.post(getByIdURL, { id : data.id }, function (data) {
					top.layer.closeAll('loading');
					if(data.success) {
						admin.putTempData('hr-person-certificate-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
				...
				top.layer.confirm(fox.translate('确定删除此'+'证书信息'+'吗？'), function (i) {
					top.layer.close(i);
	...
	function showEditApprovalForm(data) {
		if(window.pageExt.list.beforeEdit) {
			var doNext=window.pageExt.list.beforeEdit(data);
			if(!doNext) return;
		}
		var action=admin.getTempData('hr-person-certificate-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('hr-person-certificate-form-data', data);
		var area=admin.getTempData('hr-person-certificate-form-area');
		...
		admin.popupCenter({
			...
			area: ["65%",height+"px"],
			type: 2,
			id:"hr-person-certificate-form-data-win",
	...
	function showEditForm(data) {
		...
		admin.popupCenter({
			...
			type: 2,
			id:"hr-person-certificate-form-data-win",
			content: '/business/hr/person_certificate/person_cert_a_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
