...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('hr-personnel-requirement-apply-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('hr-personnel-requirement-apply-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'hr-personnel-requirement-apply-form-data-win');
			if(area==null) return;
			admin.putTempData('hr-personnel-requirement-apply-form-area', area);
	...
	function renderFormFields() {
		...
		laydate.render({
			...
			trigger:"click",
				...
				window.pageExt.form.onDatePickerChanged && window.pageExt.form.onDatePickerChanged("sdate",value, date, endDate);
			}
		});
		laydate.render({
			elem: '#edate',
			...
			trigger:"click",
				...
				window.pageExt.form.onDatePickerChanged && window.pageExt.form.onDatePickerChanged("edate",value, date, endDate);
			}
		});
	}
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('hr-personnel-requirement-apply-form-data');
		}
		...
		if (hasData) {
			...
			//设置 开始日期 显示复选框勾选
			if(formData["sdate"]) {
				$("#sdate").val(fox.dateFormat(formData["sdate"],"yyyy-MM-dd"));
			}
			//设置 结束日期 显示复选框勾选
			if(formData["edate"]) {
				$("#edate").val(fox.dateFormat(formData["edate"],"yyyy-MM-dd"));
			}
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('hr-personnel-requirement-apply-form-data');
		}
		return rawFormData;
	}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('hr-personnel-requirement-apply-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('hr-personnel-requirement-apply-form-data-win',this); });
