...
layui.define(['form', 'table', 'util', 'settings', 'admin', 'upload','foxnic','xmSelect','laydate','foxnicUpload','dropdown'],function () {
    ...
    var list={
        ...
        afterQuery : function (data) {
            for (var i = 0; i < data.length; i++) {
                ...
                if(data[i].status=="wait") {
                    ...
                    fox.disableButton($('.create-data').filter("[data-id='" + data[i].id + "']"), true);
        ...
        createData:function (ps, it){
            console.log('moreAction',ps,it);
            var btnClass="create-data";
            var btn=$('.'+btnClass).filter("[data-id='" +ps.id + "']");
            var api="/service-hr/hr-salary-action/create-data"
            top.layer.confirm(fox.translate('确定进行该操作吗？'), function (i) {
                top.layer.close(i);
                admin.post(api, ps, function (r) {
                    if (r.success) {
                        top.layer.msg("生成成功", {time: 1000});
        ...
        salaryDetail:function (data, it){
            var area=admin.getTempData('hr-salary-detail-list-area');
            ...
            admin.popupCenter({
                ...
                area: ["98%","98%"],
                type: 2,
                id:"hr-salary-detail-list-data-win",
                // content: '/business/hr/salary_action/salary_action_list.html?actionId='+data.id,
                content: '/business/hr/salary_action/salary_detail_list2.html?actionId='+data.id,
                finish: function () {
                }
            });
        },
        ...
        detail:function (data){
            ...
            admin.popupCenter({
                ...
                content: '/business/hr/salary_action/person_list.html?tplId='+data.tplId,
                finish: function () {
                }
            });
        },
