...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('hr-score-user-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('hr-score-user-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'hr-score-user-form-data-win');
			if(area==null) return;
			admin.putTempData('hr-score-user-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("relationship",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					if(window.pageExt.form.selectBoxDataTransform) {
						opts.push(window.pageExt.form.selectBoxDataTransform("relationship",{data:data[i],name:data[i].name,value:data[i].code,selected:(defaultValues.indexOf(data[i].code)!=-1 || defaultIndexs.indexOf(""+i)!=-1)},data[i],data,i));
					} else {
		...
		fox.renderSelectBox({
			el: "tplId",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("考评卷",'','cmp:form'),
			filterable: true,
			paging: true,
			pageRemote: true,
			layVerify: 'required',
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('hr-score-user-form-data');
		}
		...
		if (hasData) {
			...
			//设置  关系 设置下拉框勾选
			fox.setSelectValue4QueryApi("#relationship",formData.evaluationRelation);
			//设置  考评卷 设置下拉框勾选
			fox.setSelectValue4QueryApi("#tplId",formData.scoreTpl);
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('hr-score-user-form-data');
		}
		return rawFormData;
	}
	...
	function getFormData() {
		...
		//获取 关系 下拉框的值
		data["relationship"]=fox.getSelectedValue("relationship",true);
		//获取 考评卷 下拉框的值
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('hr-score-user-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('hr-score-user-form-data-win',this); });
