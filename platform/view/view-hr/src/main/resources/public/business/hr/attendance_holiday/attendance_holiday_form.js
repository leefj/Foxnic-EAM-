...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('hr-attendance-holiday-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('hr-attendance-holiday-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'hr-attendance-holiday-form-data-win');
			if(area==null) return;
			admin.putTempData('hr-attendance-holiday-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			el: "actionType",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("休假类型",'','cmp:form'),
			filterable: false,
			on: function(data){
				setTimeout(function () {
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('hr-attendance-holiday-form-data');
		}
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('hr-attendance-holiday-form-data');
		}
		return rawFormData;
	}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('hr-attendance-holiday-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('hr-attendance-holiday-form-data-win',this); });
