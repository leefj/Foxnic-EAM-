...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('hr-score-tpl-item-value-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('hr-score-tpl-item-value-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'hr-score-tpl-item-value-form-data-win');
			if(area==null) return;
			admin.putTempData('hr-score-tpl-item-value-form-area', area);
	...
	function renderFormFields() {
		...
		form.on('radio(itemType)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=itemType]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("itemType",data,checked);
		});
		//渲染 groupId 下拉字段
		fox.renderSelectBox({
			el: "groupId",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("评分组",'','cmp:form'),
			filterable: false,
			layVerify: 'required',
			layVerType: 'msg',
			on: function(data){
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					if(window.pageExt.form.selectBoxDataTransform) {
						opts.push(window.pageExt.form.selectBoxDataTransform("groupId",{data:data[i],name:data[i].label,value:data[i].code,selected:(defaultValues.indexOf(data[i].code)!=-1 || defaultIndexs.indexOf(""+i)!=-1)},data[i],data,i));
					} else {
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('hr-score-tpl-item-value-form-data');
		}
		...
		if (hasData) {
			...
			//设置  评分组 设置下拉框勾选
			fox.setSelectValue4QueryApi("#groupId",formData.groupId);
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('hr-score-tpl-item-value-form-data');
		}
		return rawFormData;
	}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('hr-score-tpl-item-value-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('hr-score-tpl-item-value-form-data-win',this); });
