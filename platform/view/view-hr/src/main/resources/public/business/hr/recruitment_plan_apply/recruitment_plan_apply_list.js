...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-hr/hr-recruitment-plan-apply";
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'title', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('招聘主题') , templet: function (d) { return templet('title',d.title,d);}  }
					,{ field: 'orgId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('招聘部门') , templet: function (d) { return templet('orgId',fox.getProperty(d,["organization","fullName"],0,'','orgId'),d);} }
					...
					,{ field: 'pos', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('招聘岗位') , templet: function (d) { return templet('pos',d.pos,d);}  }
					,{ field: 'personNumber', align:"right",fixed:false,  hide:false, sort: true  , title: fox.translate('招聘人数') , templet: function (d) { return templet('personNumber',d.personNumber,d);}  }
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('hr-recruitment-plan-apply-form-data', {});
					var defaultValue={};
					if(window.pageExt.list.getBpmViewConfig) {
						defaultValue=window.pageExt.list.getBpmViewConfig(obj.event);
					}
					admin.putTempData('hr-recruitment-plan-apply-form-data-form-action', "create",true);
					bpm.openProcessView(null,null,false,{"formDefinitionCode":"hr_recruitment_plan_apply"},refreshTableData,refreshRowData,"bill",defaultValue);
					break;
				case 'batch-del':
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'招聘计划申请'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'招聘计划申请'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('hr-recruitment-plan-apply-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				top.layer.load(2);
				bpm.getProcessInstanceByBill("hr_recruitment_plan_apply",{ id : data.id },function(p) {
					top.layer.closeAll('loading');
					if(p) {
						admin.putTempData('hr-recruitment-plan-apply-form-data-form-action', "edit",true);
						bpm.openProcessView(p.id,null,false,{"formDefinitionCode":"hr_recruitment_plan_apply"},refreshTableData,refreshRowData,"bill");
					} else {
				...
				admin.post(getByIdURL, { id : data.id }, function (data) {
					top.layer.closeAll('loading');
					if(data.success) {
						admin.putTempData('hr-recruitment-plan-apply-form-data-form-action', "view",true);
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('hr-recruitment-plan-apply-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('hr-recruitment-plan-apply-form-data', data);
		var area=admin.getTempData('hr-recruitment-plan-apply-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('招聘计划申请');
		...
		admin.popupCenter({
			...
			area: ["76%",height+"px"],
			type: 2,
			id:"hr-recruitment-plan-apply-form-data-win",
			content: '/business/hr/recruitment_plan_apply/recruitment_plan_apply_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
