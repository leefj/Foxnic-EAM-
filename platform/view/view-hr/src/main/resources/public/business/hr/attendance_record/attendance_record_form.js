...
function FormPage() {
	...
	function renderFormFields() {
		...
		form.on('radio(processStatus)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=processStatus]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("processStatus",data,checked);
		});
		laydate.render({
			elem: '#processTime',
			type:"date",
			...
			trigger:"click",
				...
				window.pageExt.form.onDatePickerChanged && window.pageExt.form.onDatePickerChanged("processTime",value, date, endDate);
			}
		});
	}
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('hr-attendance-record-form-data');
		}
		return rawFormData;
	}
