...
function FormPage() {
	...
	// 接口地址
	const moduleURL="/service-hr/hr-salary-tpl-item";
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('hr-salary-tpl-item-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('hr-salary-tpl-item-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'hr-salary-tpl-item-form-data-win');
			if(area==null) return;
			admin.putTempData('hr-salary-tpl-item-form-area', area);
	...
	function renderFormFields() {
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					if(window.pageExt.form.selectBoxDataTransform) {
						opts.push(window.pageExt.form.selectBoxDataTransform("code",{data:data[i],name:data[i].colName,value:data[i].colCode,selected:(defaultValues.indexOf(data[i].colCode)!=-1 || defaultIndexs.indexOf(""+i)!=-1)},data[i],data,i));
					} else {
						opts.push({data:data[i],name:data[i].colName,value:data[i].colCode,selected:(defaultValues.indexOf(data[i].colCode)!=-1 || defaultIndexs.indexOf(""+i)!=-1)});
					}
				}
				return opts;
			}
		...
		form.on('radio(includeExpression)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=includeExpression]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("includeExpression",data,checked);
		});
	}
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('hr-salary-tpl-item-form-data');
		}
		...
		if (hasData) {
			...
			//设置  编码 设置下拉框勾选
			fox.setSelectValue4QueryApi("#code",formData.salaryColumn);
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('hr-salary-tpl-item-form-data');
		}
		return rawFormData;
	}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('hr-salary-tpl-item-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('hr-salary-tpl-item-form-data-win',this); });
