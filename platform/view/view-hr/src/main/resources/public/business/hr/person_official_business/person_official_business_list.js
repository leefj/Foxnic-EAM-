...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-hr/hr-person-official-business";
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'leaveDate', align:"right", fixed:false, hide:false, sort: true   ,title: fox.translate('出差日期') ,templet: function (d) { return templet('leaveDate',fox.dateFormat(d.leaveDate,"yyyy-MM-dd"),d); }  }
					...
					,{ field: 'location', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('地点') , templet: function (d) { return templet('location',d.location,d);}  }
					,{ field: 'days', align:"right",fixed:false,  hide:false, sort: true  , title: fox.translate('天数') , templet: function (d) { return templet('days',d.days,d);}  }
					,{ field: 'money', align:"right",fixed:false,  hide:false, sort: true  , title: fox.translate('申请费用') , templet: function (d) { return templet('money',d.money,d);}  }
	...
	function initSearchFields() {
		...
		laydate.render({
			...
			trigger:"click",
				...
				setTimeout(function () {
					window.pageExt.list.onDatePickerChanged && window.pageExt.list.onDatePickerChanged("leaveDate",value, date, endDate);
				},1);
			}
		});
		laydate.render({
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('hr-person-official-business-form-data', {});
					var defaultValue={};
					if(window.pageExt.list.getBpmViewConfig) {
						defaultValue=window.pageExt.list.getBpmViewConfig(obj.event);
					}
					admin.putTempData('hr-person-official-business-form-data-form-action', "create",true);
					bpm.openProcessView(null,null,false,{"formDefinitionCode":"hr_person_official_business"},refreshTableData,refreshRowData,"bill",defaultValue);
					break;
				case 'batch-del':
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'出差申请'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'出差申请'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('hr-person-official-business-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				top.layer.load(2);
				bpm.getProcessInstanceByBill("hr_person_official_business",{ id : data.id },function(p) {
					top.layer.closeAll('loading');
					if(p) {
						admin.putTempData('hr-person-official-business-form-data-form-action', "edit",true);
						bpm.openProcessView(p.id,null,false,{"formDefinitionCode":"hr_person_official_business"},refreshTableData,refreshRowData,"bill");
					} else {
				...
				admin.post(getByIdURL, { id : data.id }, function (data) {
					top.layer.closeAll('loading');
					if(data.success) {
						admin.putTempData('hr-person-official-business-form-data-form-action', "view",true);
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('hr-person-official-business-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('hr-person-official-business-form-data', data);
		var area=admin.getTempData('hr-person-official-business-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('出差申请');
		...
		admin.popupCenter({
			...
			area: ["75%",height+"px"],
			type: 2,
			id:"hr-person-official-business-form-data-win",
			content: '/business/hr/person_official_business/person_official_business_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
