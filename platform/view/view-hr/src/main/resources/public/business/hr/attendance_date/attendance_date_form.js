...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('hr-attendance-date-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('hr-attendance-date-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'hr-attendance-date-form-data-win');
			if(area==null) return;
			admin.putTempData('hr-attendance-date-form-area', area);
	...
	function renderFormFields() {
		...
		form.on('radio(week)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=week]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("week",data,checked);
		});
		form.on('radio(statutoryHoliday)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=statutoryHoliday]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("statutoryHoliday",data,checked);
		});
		form.on('radio(attendancSign)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=attendancSign]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("attendancSign",data,checked);
		});
		laydate.render({
			elem: '#confSDate',
			type:"date",
			format:"yyyy-MM-dd",
			value:$('#confSDate').val()?$('#confSDate').val():new Date(),
			trigger:"click",
			done: function(value, date, endDate){
				window.pageExt.form.onDatePickerChanged && window.pageExt.form.onDatePickerChanged("confSDate",value, date, endDate);
			}
		});
		laydate.render({
			elem: '#confEDate',
			type:"date",
			format:"yyyy-MM-dd",
			value:$('#confEDate').val()?$('#confEDate').val():new Date(),
			trigger:"click",
			done: function(value, date, endDate){
				window.pageExt.form.onDatePickerChanged && window.pageExt.form.onDatePickerChanged("confEDate",value, date, endDate);
			}
		});
		form.on('radio(confCreateDate)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=confCreateDate]:checked').each(function() {
				checked.push($(this).val());
			});
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('hr-attendance-date-form-data');
		}
		...
		if (hasData) {
			...
			//设置 开始日期 显示复选框勾选
			if(formData["confSDate"]) {
				$("#confSDate").val(fox.dateFormat(formData["confSDate"],"yyyy-MM-dd"));
			}
			//设置 结束日期 显示复选框勾选
			if(formData["confEDate"]) {
				$("#confEDate").val(fox.dateFormat(formData["confEDate"],"yyyy-MM-dd"));
			}
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('hr-attendance-date-form-data');
		}
		return rawFormData;
	}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('hr-attendance-date-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('hr-attendance-date-form-data-win',this); });
