...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('hr-salary-action-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('hr-salary-action-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'hr-salary-action-form-data-win');
			if(area==null) return;
			admin.putTempData('hr-salary-action-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			el: "actionMonth",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("月份",'','cmp:form'),
			filterable: true,
			paging: true,
			pageRemote: true,
			layVerify: 'required',
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("actionMonth",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					if(window.pageExt.form.selectBoxDataTransform) {
						opts.push(window.pageExt.form.selectBoxDataTransform("actionMonth",{data:data[i],name:data[i].name,value:data[i].code,selected:(defaultValues.indexOf(data[i].code)!=-1 || defaultIndexs.indexOf(""+i)!=-1)},data[i],data,i));
					} else {
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('hr-salary-action-form-data');
		}
		...
		if (hasData) {
			...
			//设置  月份 设置下拉框勾选
			fox.setSelectValue4QueryApi("#actionMonth",formData.salaryMonth);
			//设置  模版 设置下拉框勾选
			fox.setSelectValue4QueryApi("#tplId",formData.salaryTpl);
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('hr-salary-action-form-data');
		}
		return rawFormData;
	}
	...
	function getFormData() {
		...
		//获取 月份 下拉框的值
		data["actionMonth"]=fox.getSelectedValue("actionMonth",false);
		//获取 模版 下拉框的值
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('hr-salary-action-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('hr-salary-action-form-data-win',this); });
