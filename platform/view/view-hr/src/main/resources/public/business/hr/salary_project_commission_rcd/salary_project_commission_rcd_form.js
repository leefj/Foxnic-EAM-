...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('hr-salary-project-commission-rcd-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('hr-salary-project-commission-rcd-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'hr-salary-project-commission-rcd-form-data-win');
			if(area==null) return;
			admin.putTempData('hr-salary-project-commission-rcd-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			el: "projectCode",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("项目编码",'','cmp:form'),
			filterable: true,
			paging: true,
			pageRemote: true,
			layVerify: 'required',
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("projectCode",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			searchField: "name", //请自行调整用于搜索的字段名称
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					if(!data[i]) continue;
					if(window.pageExt.form.selectBoxDataTransform) {
						opts.push(window.pageExt.form.selectBoxDataTransform("projectCode",{data:data[i],name:data[i].name,value:data[i].code,selected:(defaultValues.indexOf(data[i].code)!=-1 || defaultIndexs.indexOf(""+i)!=-1)},data[i],data,i));
					} else {
		...
		laydate.render({
			...
			trigger:"click",
				...
				window.pageExt.form.onDatePickerChanged && window.pageExt.form.onDatePickerChanged("rcdDate",value, date, endDate);
			}
		});
	    //渲染图片字段
		foxup.render({
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('hr-salary-project-commission-rcd-form-data');
		}
		...
		if (hasData) {
			...
			//设置 记录日期 显示复选框勾选
			if(formData["rcdDate"]) {
				$("#rcdDate").val(fox.dateFormat(formData["rcdDate"],"yyyy-MM-dd"));
			}
			...
			//设置  项目编码 设置下拉框勾选
			fox.setSelectValue4QueryApi("#projectCode",formData.project);
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('hr-salary-project-commission-rcd-form-data');
		}
		return rawFormData;
	}
	...
	function getFormData() {
		...
		//获取 项目编码 下拉框的值
		data["projectCode"]=fox.getSelectedValue("projectCode",false);
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('hr-salary-project-commission-rcd-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('hr-salary-project-commission-rcd-form-data-win',this); });
