...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('hr-score-tpl-item-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('hr-score-tpl-item-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'hr-score-tpl-item-form-data-win');
			if(area==null) return;
			admin.putTempData('hr-score-tpl-item-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			el: "groupId",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("指标",'','cmp:form'),
			filterable: false,
			layVerify: 'required',
			layVerType: 'msg',
			on: function(data){
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('hr-score-tpl-item-form-data');
		}
		...
		if (hasData) {
			...
			//设置  指标 设置下拉框勾选
			fox.setSelectValue4QueryApi("#groupId",formData.scoreIndicator);
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('hr-score-tpl-item-form-data');
		}
		return rawFormData;
	}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('hr-score-tpl-item-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('hr-score-tpl-item-form-data-win',this); });
