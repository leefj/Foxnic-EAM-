...
function FormPage() {
	...
	// 接口地址
	const moduleURL="/service-hr/hr-assessment-indicator";
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('hr-assessment-indicator-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('hr-assessment-indicator-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'hr-assessment-indicator-form-data-win');
			if(area==null) return;
			admin.putTempData('hr-assessment-indicator-form-area', area);
	...
	function renderFormFields() {
		...
		//渲染 tplIndicatorId 下拉字段
		fox.renderSelectBox({
			el: "tplIndicatorId",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("指标库",'','cmp:form'),
			filterable: true,
			paging: true,
			pageRemote: true,
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("tplIndicatorId",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					if(window.pageExt.form.selectBoxDataTransform) {
						opts.push(window.pageExt.form.selectBoxDataTransform("tplIndicatorId",{data:data[i],name:data[i].name,value:data[i].id,selected:(defaultValues.indexOf(data[i].id)!=-1 || defaultIndexs.indexOf(""+i)!=-1)},data[i],data,i));
					} else {
		...
		form.on('radio(property)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=property]:checked').each(function() {
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('hr-assessment-indicator-form-data');
		}
		...
		if (hasData) {
			...
			//设置  指标库 设置下拉框勾选
			fox.setSelectValue4QueryApi("#tplIndicatorId",formData.indicatorLib);
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('hr-assessment-indicator-form-data');
		}
		return rawFormData;
	}
	...
	function getFormData() {
		...
		//获取 指标库 下拉框的值
		data["tplIndicatorId"]=fox.getSelectedValue("tplIndicatorId",false);
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('hr-assessment-indicator-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('hr-assessment-indicator-form-data-win',this); });
