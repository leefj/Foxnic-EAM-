...
function FormPage() {
	...
	// 接口地址
	const moduleURL="/service-hr/hr-person-absence-apply";
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('hr-person-absence-apply-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('hr-person-absence-apply-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'hr-person-absence-apply-form-data-win');
			if(area==null) return;
			admin.putTempData('hr-person-absence-apply-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			el: "applyType",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("请假类型",'','cmp:form'),
			filterable: true,
			paging: true,
			pageRemote: true,
			layVerify: 'required',
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("applyType",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					if(!data[i]) continue;
					if(window.pageExt.form.selectBoxDataTransform) {
						opts.push(window.pageExt.form.selectBoxDataTransform("applyType",{data:data[i],name:data[i].name,value:data[i].code,selected:(defaultValues.indexOf(data[i].code)!=-1 || defaultIndexs.indexOf(""+i)!=-1)},data[i],data,i));
					} else {
		...
		laydate.render({
			...
			trigger:"click",
				...
				window.pageExt.form.onDatePickerChanged && window.pageExt.form.onDatePickerChanged("applySDate",value, date, endDate);
			}
		});
		laydate.render({
			elem: '#applyEDate',
			...
			trigger:"click",
				...
				window.pageExt.form.onDatePickerChanged && window.pageExt.form.onDatePickerChanged("applyEDate",value, date, endDate);
			}
		});
	    //渲染图片字段
		foxup.render({
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('hr-person-absence-apply-form-data');
		}
		...
		if (hasData) {
			...
			//设置 开始日期 显示复选框勾选
			if(formData["applySDate"]) {
				$("#applySDate").val(fox.dateFormat(formData["applySDate"],"yyyy-MM-dd"));
			}
			//设置 结束日期 显示复选框勾选
			if(formData["applyEDate"]) {
				$("#applyEDate").val(fox.dateFormat(formData["applyEDate"],"yyyy-MM-dd"));
			}


			//设置  请假类型 设置下拉框勾选
			fox.setSelectValue4QueryApi("#applyType",formData.attendanceType);
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('hr-person-absence-apply-form-data');
		}
		return rawFormData;
	}
	...
	function getFormData() {
		...
		//获取 请假类型 下拉框的值
		data["applyType"]=fox.getSelectedValue("applyType",false);
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('hr-person-absence-apply-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('hr-person-absence-apply-form-data-win',this); });
