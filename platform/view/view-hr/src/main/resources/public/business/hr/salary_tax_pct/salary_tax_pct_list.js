...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-hr/hr-salary-tax-pct";
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'min', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('最小') , templet: function (d) { return templet('min',d.min,d);}  }
					,{ field: 'minInclude', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('最小包含'), templet:function (d){ return templet('minInclude',fox.getEnumText(RADIO_MININCLUDE_DATA,d.minInclude,'','minInclude'),d);}}
					,{ field: 'max', align:"right",fixed:false,  hide:false, sort: true  , title: fox.translate('最大') , templet: function (d) { return templet('max',d.max,d);}  }
					,{ field: 'maxInclude', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('最大包含'), templet:function (d){ return templet('maxInclude',fox.getEnumText(RADIO_MAXINCLUDE_DATA,d.maxInclude,'','maxInclude'),d);}}
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('hr-salary-tax-pct-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('hr-salary-tax-pct-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'薪酬税率'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'薪酬税率'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('hr-salary-tax-pct-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				admin.post(getByIdURL, { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('hr-salary-tax-pct-form-data-form-action', "edit",true);
				...
				admin.post(getByIdURL, { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('hr-salary-tax-pct-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
				...
				top.layer.confirm(fox.translate('确定删除此'+'薪酬税率'+'吗？'), function (i) {
					top.layer.close(i);
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('hr-salary-tax-pct-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('hr-salary-tax-pct-form-data', data);
		var area=admin.getTempData('hr-salary-tax-pct-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('薪酬税率');
		...
		admin.popupCenter({
			...
			area: ["65%",height+"px"],
			type: 2,
			id:"hr-salary-tax-pct-form-data-win",
			content: '/business/hr/salary_tax_pct/salary_tax_pct_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
