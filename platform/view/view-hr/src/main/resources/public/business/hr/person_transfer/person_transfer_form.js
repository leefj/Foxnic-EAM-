...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('hr-person-transfer-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('hr-person-transfer-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'hr-person-transfer-form-data-win');
			if(area==null) return;
			admin.putTempData('hr-person-transfer-form-area', area);
	...
	function renderFormFields() {
		...
		laydate.render({
			...
			trigger:"click",
				...
				window.pageExt.form.onDatePickerChanged && window.pageExt.form.onDatePickerChanged("transferDate",value, date, endDate);
			}
		});
		...
		fox.renderSelectBox({
			el: "positionCode",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("岗位",'','cmp:form'),
			filterable: true,
			paging: true,
			pageRemote: true,
			on: function(data){
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('hr-person-transfer-form-data');
		}
		...
		if (hasData) {
			...
			if(formData["transferDate"]) {
				$("#transferDate").val(fox.dateFormat(formData["transferDate"],"yyyy-MM-dd"));
			}
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('hr-person-transfer-form-data');
		}
		return rawFormData;
	}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('hr-person-transfer-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('hr-person-transfer-form-data-win',this); });
