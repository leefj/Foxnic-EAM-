...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-oa/oa-schedule-plan";
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'rank', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('优先级'), templet:function (d){ return templet('rank',fox.getEnumText(CHECK_RANK_DATA,d.rank,'','rank'),d);}}
					,{ field: 'fTime', align:"right", fixed:false, hide:false, sort: true   ,title: fox.translate('开始时间') ,templet: function (d) { return templet('fTime',fox.dateFormat(d.fTime,"yyyy-MM-dd HH:mm:ss"),d); }  }
	...
	function refreshTableData(sortField,sortType,reset) {
		...
		var value = {};
		value.rank={ inputType:"check_box", value: getSelectedValue("#rank","value"), label:getSelectedValue("#rank","nameStr") };
		value.fTime={ inputType:"date_input", value: $("#fTime").val() ,matchType:"auto"};
		var ps={searchField:"$composite"};
		if(window.pageExt.list.beforeQuery){
	...
	function initSearchFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.list.onSelectBoxChanged && window.pageExt.list.onSelectBoxChanged("rank",data.arr,data.change,data.isAdd);
				},1);
			},
				...
				transform:function(data) {
					...
					for (var i = 0; i < data.length; i++) {
						if(window.pageExt.list.selectBoxDataTransform) {
							opts.push(window.pageExt.list.selectBoxDataTransform("rank",{data:data[i],name:data[i].text,value:data[i].code},data[i],data,i));
						} else {
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('oa-schedule-plan-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('oa-schedule-plan-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'日程安排'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'日程安排'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('oa-schedule-plan-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				admin.post(getByIdURL, { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('oa-schedule-plan-form-data-form-action', "edit",true);
				...
				admin.post(getByIdURL, { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('oa-schedule-plan-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
				...
				top.layer.confirm(fox.translate('确定删除此'+'日程安排'+'吗？'), function (i) {
					top.layer.close(i);
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('oa-schedule-plan-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('oa-schedule-plan-form-data', data);
		var area=admin.getTempData('oa-schedule-plan-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('日程安排');
		...
		admin.popupCenter({
			...
			area: ["80%",height+"px"],
			type: 2,
			id:"oa-schedule-plan-form-data-win",
			content: '/business/oa/schedule_plan/schedule_plan_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
