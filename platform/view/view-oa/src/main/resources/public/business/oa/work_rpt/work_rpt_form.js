...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('oa-work-rpt-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('oa-work-rpt-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'oa-work-rpt-form-data-win');
			if(area==null) return;
			admin.putTempData('oa-work-rpt-form-area', area);
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('oa-work-rpt-form-data');
		}
		...
		if (hasData) {
			...
			//设置 开始时间 显示复选框勾选
			if(formData["fTime"]) {
				$("#fTime").val(fox.dateFormat(formData["fTime"],"yyyy-MM-dd"));
			}
			//设置 结束时间 显示复选框勾选
			if(formData["tTime"]) {
				$("#tTime").val(fox.dateFormat(formData["tTime"],"yyyy-MM-dd"));
			}
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('oa-work-rpt-form-data');
		}
		return rawFormData;
	}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('oa-work-rpt-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('oa-work-rpt-form-data-win',this); });
