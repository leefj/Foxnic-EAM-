	...
	function renderTable() {
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				url: "/service-oa/oa-netdisk-my-share/query-share-member",
				height: 'full-'+(h+28),
				limit: 50,
				where: ps,
				cols: [[
					...
					,{ field: 'userId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('成员') , templet: function (d) { return templet('userId',d.userId,d);}  }
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					var fdData=[];
					var fdObj={fdId:FD_ID,fdType:"file"};
					fdData.push(fdObj);
					$("#userId").val("");
					var userIdDialogOptions={
						field:"userId",
						...
						callback:function(param,result){
							...
							if(userIds.length>0){
								admin.post("/service-oa/oa-netdisk-virtual-fd/share", ps, function (rs) {
									if (rs.success) {
										fox.showMessage(rs);
				...
				case 'remove':
					...
					if(ids.length==0) {
						top.layer.msg(fox.translate('请选择需要删除的成员!'));
						return;
					}
					//调用批量删除接口
					top.layer.confirm(fox.translate('确定删除已选中的成员吗？'), function (i) {
						top.layer.close(i);
						admin.post("/service-oa/oa-netdisk-my-share/remove-share-member", { ids: ids }, function (rs) {
							if (rs.success) {
								fox.showMessage(rs);
								refreshTableData();
							} else {
						...
						},{delayLoading:200,elms:[$("#remove-button")]});
					});
					break;
			};
		});
