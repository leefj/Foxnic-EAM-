...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-oa/oa-netdisk-recycle";
	...
	const restoreURL=moduleURL+'/restore';
	//
	var dataTable=null;
	var sort=null;
	...
	function refreshTableData(sortField,sortType,reset) {
		...
		value.delTime={ inputType:"date_input", value: $("#delTime").val() ,matchType:"auto"};
	...
	function initSearchFields() {
		...
		laydate.render({
			...
			trigger:"click",
				...
				setTimeout(function () {
					window.pageExt.list.onDatePickerChanged && window.pageExt.list.onDatePickerChanged("delTime",value, date, endDate);
				},1);
			}
		});
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('oa-netdisk-recycle-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('oa-netdisk-recycle-form-data-form-action', "create",true);
            showEditForm(data);
        };
		...
		function restore(selected) {
			...
			if(ids.length==0) {
				top.layer.msg(fox.translate('请选择要恢复的内容'));
				return;
			}
			//调用批量删除接口
			top.layer.confirm(fox.translate('确定恢复已选中的内容吗？'), function (i) {
				top.layer.close(i);
				admin.post(restoreURL, { ids: ids }, function (data) {
					if (data.success) {
						fox.showMessage(data);
						refreshTableData();
					} else {
				...
				},{delayLoading:200,elms:[$("#restore-file-button")]});
			});
		}
        ...
        function batchDelete(selected) {
			...
			var ids=getCheckedList("id");
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'回收站'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'回收站'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('oa-netdisk-recycle-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				top.layer.load(2);
				top.layer.load(2);
				admin.post(getByIdURL, { id : data.id }, function (data) {
					...
					if(data.success) {
						admin.putTempData('oa-netdisk-recycle-form-data-form-action', "edit",true);
				...
				admin.post(getByIdURL, { id : data.id }, function (data) {
					top.layer.closeAll('loading');
					if(data.success) {
						admin.putTempData('oa-netdisk-recycle-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('oa-netdisk-recycle-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('oa-netdisk-recycle-form-data', data);
		var area=admin.getTempData('oa-netdisk-recycle-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('回收站');
		...
		admin.popupCenter({
			...
			type: 2,
			id:"oa-netdisk-recycle-form-data-win",
			content: '/business/oa/netdisk_recycle/netdisk_recycle_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
