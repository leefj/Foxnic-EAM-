...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-oa/oa-netdisk-oper-record";
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'fdId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('对象') , templet: function (d) { return templet('fdId',d.fdId,d);}  }
					,{ field: 'fdType', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('对象类型') , templet: function (d) { return templet('fdType',d.fdType,d);}  }
					,{ field: 'operContent', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('操作内容') , templet: function (d) { return templet('operContent',d.operContent,d);}  }
					,{ field: 'operUserId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('操作人') , templet: function (d) { return templet('operUserId',fox.getProperty(d,["user","name"],0,'','operUserId'),d);} }
	...
	function refreshTableData(sortField,sortType,reset) {
		...
		var value = {};
		value.operUserId={ inputType:"button",value: $("#operUserId").val(),fillBy:["user","name"] ,label:$("#operUserId-button").text() };
		value.operTime={ inputType:"date_input", begin: $("#operTime-begin").val(), end: $("#operTime-end").val() ,matchType:"auto" };
		var ps={searchField:"$composite"};
		if(window.pageExt.list.beforeQuery){
	...
	function initSearchFields() {
		...
		laydate.render({
			...
			trigger:"click",
				...
				setTimeout(function () {
					window.pageExt.list.onDatePickerChanged && window.pageExt.list.onDatePickerChanged("operTime",value, date, endDate);
				},1);
			}
		});
		laydate.render({
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('oa-netdisk-oper-record-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('oa-netdisk-oper-record-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'操作记录'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'操作记录'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('oa-netdisk-oper-record-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				top.layer.load(2);
				top.layer.load(2);
				admin.post(getByIdURL, { id : data.id }, function (data) {
					...
					if(data.success) {
						admin.putTempData('oa-netdisk-oper-record-form-data-form-action', "edit",true);
				...
				admin.post(getByIdURL, { id : data.id }, function (data) {
					top.layer.closeAll('loading');
					if(data.success) {
						admin.putTempData('oa-netdisk-oper-record-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
				...
				top.layer.confirm(fox.translate('确定删除此'+'操作记录'+'吗？'), function (i) {
					top.layer.close(i);
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('oa-netdisk-oper-record-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('oa-netdisk-oper-record-form-data', data);
		var area=admin.getTempData('oa-netdisk-oper-record-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('操作记录');
		...
		admin.popupCenter({
			...
			type: 2,
			id:"oa-netdisk-oper-record-form-data-win",
			content: '/business/oa/netdisk_oper_record/netdisk_oper_record_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
