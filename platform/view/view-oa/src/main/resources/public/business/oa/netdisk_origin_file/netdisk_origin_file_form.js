...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('oa-netdisk-origin-file-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('oa-netdisk-origin-file-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'oa-netdisk-origin-file-form-data-win');
			if(area==null) return;
			admin.putTempData('oa-netdisk-origin-file-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			el: "fileType",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("文件类型",'','cmp:form'),
			filterable: false,
			layVerify: 'required',
			layVerType: 'msg',
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("fileType",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			transform:function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					if(window.pageExt.form.selectBoxDataTransform) {
						opts.push(window.pageExt.form.selectBoxDataTransform("fileType",{data:data[i],name:data[i].text,value:data[i].code,selected:(defaultValues.indexOf(data[i].code)!=-1 || defaultIndexs.indexOf(""+i)!=-1)},data[i],data,i));
					} else {
		...
		form.on('radio(encrypted)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=encrypted]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("encrypted",data,checked);
		});
		laydate.render({
			elem: '#lastVisitTime',
			type:"date",
			...
			trigger:"click",
				...
				window.pageExt.form.onDatePickerChanged && window.pageExt.form.onDatePickerChanged("lastVisitTime",value, date, endDate);
			}
		});
	}
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('oa-netdisk-origin-file-form-data');
		}
		...
		if (hasData) {
			...
			//设置  文件类型 设置下拉框勾选
			fox.setSelectValue4Enum("#fileType",formData.fileType,SELECT_FILETYPE_DATA);
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('oa-netdisk-origin-file-form-data');
		}
		return rawFormData;
	}
	...
	function getFormData() {
		...
		//获取 文件类型 下拉框的值
		data["fileType"]=fox.getSelectedValue("fileType",false);
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('oa-netdisk-origin-file-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('oa-netdisk-origin-file-form-data-win',this); });
