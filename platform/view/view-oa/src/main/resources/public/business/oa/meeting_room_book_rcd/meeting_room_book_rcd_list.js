...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-oa/oa-meeting-room-book-rcd";
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'roomId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('会议室'), templet: function (d) { return templet('roomId' ,fox.joinLabel(d.meetingRoom,"name",',','','roomId'),d);}}
					...
					,{ field: 'orgId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('预定部门') , templet: function (d) { return templet('orgId',fox.getProperty(d,["org","fullName"],0,'','orgId'),d);} }
					...
					,{ field: 'bookerId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('预定人') , templet: function (d) { return templet('bookerId',fox.getProperty(d,["booker","name"],0,'','bookerId'),d);} }
	...
	function refreshTableData(sortField,sortType,reset) {
		...
		value.roomId={ inputType:"select_box", value: getSelectedValue("#roomId","value") ,fillBy:["meetingRoom"]  , label:getSelectedValue("#roomId","nameStr") };
	...
	function initSearchFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.list.onSelectBoxChanged && window.pageExt.list.onSelectBoxChanged("roomId",data.arr,data.change,data.isAdd);
				},1);
			},
			paging: true,
			pageRemote: true,
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					if(window.pageExt.list.selectBoxDataTransform) {
						opts.push(window.pageExt.list.selectBoxDataTransform("roomId",{data:data[i],name:data[i].name,value:data[i].id},data[i],data,i));
					} else {
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('oa-meeting-room-book-rcd-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('oa-meeting-room-book-rcd-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'会议室预定'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'会议室预定'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('oa-meeting-room-book-rcd-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				admin.post(getByIdURL, { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('oa-meeting-room-book-rcd-form-data-form-action', "edit",true);
				...
				admin.post(getByIdURL, { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('oa-meeting-room-book-rcd-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
				...
				top.layer.confirm(fox.translate('确定删除此'+'会议室预定'+'吗？'), function (i) {
					top.layer.close(i);
				...
				window.pageExt.list.sure(data,this);
			}
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('oa-meeting-room-book-rcd-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('oa-meeting-room-book-rcd-form-data', data);
		var area=admin.getTempData('oa-meeting-room-book-rcd-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('会议室预定');
		...
		admin.popupCenter({
			...
			area: ["80%",height+"px"],
			type: 2,
			id:"oa-meeting-room-book-rcd-form-data-win",
			content: '/business/oa/meeting_room_book_rcd/meeting_room_book_rcd_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
