...
function FormPage() {
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			el: "vehicleStatus",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("车辆状态",'','cmp:form'),
			filterable: false,
			layVerify: 'required',
			layVerType: 'msg',
			on: function(data){
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					if(window.pageExt.form.selectBoxDataTransform) {
						opts.push(window.pageExt.form.selectBoxDataTransform("vehicleStatus",{data:data[i],name:data[i].label,value:data[i].code,selected:(defaultValues.indexOf(data[i].code)!=-1 || defaultIndexs.indexOf(""+i)!=-1)},data[i],data,i));
					} else {
		...
		fox.renderSelectBox({
			el: "type",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("车辆类型",'','cmp:form'),
			filterable: false,
			layVerify: 'required',
			layVerType: 'msg',
			on: function(data){
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('oa-vehicle-info-form-data');
		}
		return rawFormData;
	}
