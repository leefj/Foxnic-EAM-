...
layui.define(['form', 'table', 'util', 'settings', 'admin', 'upload','foxnic','xmSelect','laydate','foxnicUpload','dropdown'],function () {
    ...
    var list={
        ...
        afterQuery : function (data) {
            for (var i = 0; i < data.length; i++) {
                ...
                if(data[i].status=="apply") {
                    ...
                    console.log('done')
                    fox.disableButton($('.sure-button').filter("[data-id='" + data[i].id + "']"), true);
        ...
        sure:function (item){
            console.log('sure',item);
            top.layer.confirm(fox.translate('确定确认预约操作吗？'), function (i) {
                top.layer.close(i);
                admin.post(moduleURL+"/sure", { id: item.id }, function (data) {
                    if (data.success) {
                        window.module.refreshTableData();
                    }
                    fox.showMessage(data);
                },{delayLoading:200,elms:[$("#sure-button")]});
            });
        ...
        cancel:function (item){
            ...
            top.layer.confirm(fox.translate('确定取消预定操作吗？'), function (i) {
                top.layer.close(i);
                admin.post(moduleURL+"/cancel", { id: item.id }, function (data) {
                    if (data.success) {
                        window.module.refreshTableData();
                    }
                    fox.showMessage(data);
                },{delayLoading:200,elms:[$("#cancel-button")]});
            });
        },
        finish:function (item){
            console.log('finish',item);
            top.layer.confirm(fox.translate('确定完成预定操作吗？'), function (i) {
                top.layer.close(i);
                admin.post(moduleURL+"/finish", { id: item.id }, function (data) {
                    if (data.success) {
                        window.module.refreshTableData();
                    }
                    fox.showMessage(data);
