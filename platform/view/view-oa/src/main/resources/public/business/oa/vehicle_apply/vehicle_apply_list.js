...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-oa/oa-vehicle-apply";
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'orgId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('部门') , templet: function (d) { return templet('orgId',fox.getProperty(d,["useOrganization","fullName"],0,'','orgId'),d);} }
					,{ field: 'receiverId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('领用人') , templet: function (d) { return templet('receiverId',fox.getProperty(d,["receiver","name"],0,'','receiverId'),d);} }
					...
					,{ field: 'ifReturn', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('是否归还'), templet:function (d){ return templet('ifReturn',fox.getEnumText(SELECT_IFRETURN_DATA,d.ifReturn,'','ifReturn'),d);}}
					,{ field: 'vehicleId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('车辆') , templet: function (d) { return templet('vehicleId',d.vehicleId,d);}  }
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('oa-vehicle-apply-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('oa-vehicle-apply-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'车辆申请'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'车辆申请'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('oa-vehicle-apply-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				admin.post(getByIdURL, { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('oa-vehicle-apply-form-data-form-action', "edit",true);
				...
				admin.post(getByIdURL, { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('oa-vehicle-apply-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
				...
				top.layer.confirm(fox.translate('确定删除此'+'车辆申请'+'吗？'), function (i) {
					top.layer.close(i);
				...
				window.pageExt.list.returnData(data,this);
			}
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('oa-vehicle-apply-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('oa-vehicle-apply-form-data', data);
		var area=admin.getTempData('oa-vehicle-apply-form-area');
		...
		admin.popupCenter({
			...
			area: ["98%",height+"px"],
			type: 2,
			id:"oa-vehicle-apply-form-data-win",
			content: '/business/oa/vehicle_apply/vehicle_apply_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
