...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('oa-schedule-plan-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('oa-schedule-plan-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'oa-schedule-plan-form-data-win');
			if(area==null) return;
			admin.putTempData('oa-schedule-plan-form-area', area);
	...
	function renderFormFields() {
		...
		form.on('radio(rank)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=rank]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("rank",data,checked);
		});
		laydate.render({
			elem: '#fTime',
			type:"datetime",
			...
			trigger:"click",
				...
				window.pageExt.form.onDatePickerChanged && window.pageExt.form.onDatePickerChanged("fTime",value, date, endDate);
			}
		});
		laydate.render({
			elem: '#tTime',
			...
			trigger:"click",
				...
				window.pageExt.form.onDatePickerChanged && window.pageExt.form.onDatePickerChanged("tTime",value, date, endDate);
			}
		});
		//渲染 remind 下拉字段
		fox.renderSelectBox({
			el: "remind",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("提醒",'','cmp:form'),
			filterable: false,
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("remind",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					if(window.pageExt.form.selectBoxDataTransform) {
						opts.push(window.pageExt.form.selectBoxDataTransform("remind",{data:data[i],name:data[i].label,value:data[i].code,selected:(defaultValues.indexOf(data[i].code)!=-1 || defaultIndexs.indexOf(""+i)!=-1)},data[i],data,i));
					} else {
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('oa-schedule-plan-form-data');
		}
		...
		if (hasData) {
			...
			//设置 开始时间 显示复选框勾选
			if(formData["fTime"]) {
				$("#fTime").val(fox.dateFormat(formData["fTime"],"yyyy-MM-dd HH:mm:ss"));
			}
			//设置 结束时间 显示复选框勾选
			if(formData["tTime"]) {
				$("#tTime").val(fox.dateFormat(formData["tTime"],"yyyy-MM-dd HH:mm:ss"));
			}


			//设置  提醒 设置下拉框勾选
			fox.setSelectValue4QueryApi("#remind",formData.scheduleRemind);
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('oa-schedule-plan-form-data');
		}
		return rawFormData;
	}
	...
	function getFormData() {
		...
		//获取 提醒 下拉框的值
		data["remind"]=fox.getSelectedValue("remind",false);
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('oa-schedule-plan-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('oa-schedule-plan-form-data-win',this); });
