...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('oa-netdisk-menu-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('oa-netdisk-menu-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'oa-netdisk-menu-form-data-win');
			if(area==null) return;
			admin.putTempData('oa-netdisk-menu-form-area', area);
	...
	function renderFormFields() {
		...
		form.on('radio(showValue)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=showValue]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("showValue",data,checked);
		});
	}
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('oa-netdisk-menu-form-data');
		}
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('oa-netdisk-menu-form-data');
		}
		return rawFormData;
	}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('oa-netdisk-menu-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('oa-netdisk-menu-form-data-win',this); });
