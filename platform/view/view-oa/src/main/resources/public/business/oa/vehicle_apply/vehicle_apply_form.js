...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('oa-vehicle-apply-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('oa-vehicle-apply-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'oa-vehicle-apply-form-data-win');
			if(area==null) return;
			admin.putTempData('oa-vehicle-apply-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			el: "ifReturn",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("是否归还",'','cmp:form'),
			filterable: false,
			on: function(data){
				setTimeout(function () {
			...
			transform:function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					if(window.pageExt.form.selectBoxDataTransform) {
						opts.push(window.pageExt.form.selectBoxDataTransform("ifReturn",{data:data[i],name:data[i].text,value:data[i].code,selected:(defaultValues.indexOf(data[i].code)!=-1 || defaultIndexs.indexOf(""+i)!=-1)},data[i],data,i));
					} else {
						opts.push({data:data[i],name:data[i].text,value:data[i].code,selected:(defaultValues.indexOf(data[i].code)!=-1 || defaultIndexs.indexOf(""+i)!=-1)});
					}
				}
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('oa-vehicle-apply-form-data');
		}
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('oa-vehicle-apply-form-data');
		}
		return rawFormData;
	}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('oa-vehicle-apply-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('oa-vehicle-apply-form-data-win',this); });
