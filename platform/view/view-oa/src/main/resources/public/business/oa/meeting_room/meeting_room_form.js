...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('oa-meeting-room-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('oa-meeting-room-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'oa-meeting-room-form-data-win');
			if(area==null) return;
			admin.putTempData('oa-meeting-room-form-area', area);
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('oa-meeting-room-form-data');
		}
		...
		if (hasData) {
			...
			//设置  位置 设置下拉框勾选
			fox.setSelectValue4QueryApi("#positionId",formData.meetingRoomPosition);
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('oa-meeting-room-form-data');
		}
		return rawFormData;
	}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('oa-meeting-room-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('oa-meeting-room-form-data-win',this); });
