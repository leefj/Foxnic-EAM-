...
function FormPage() {
	...
	// 接口地址
	const moduleURL="/service-oa/oa-crm-customer-review";
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('oa-crm-customer-review-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('oa-crm-customer-review-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'oa-crm-customer-review-form-data-win');
			if(area==null) return;
			admin.putTempData('oa-crm-customer-review-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			el: "customerId",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("客户",'','cmp:form'),
			filterable: true,
			paging: true,
			pageRemote: true,
			on: function(data){
		...
		laydate.render({
			...
			trigger:"click",
				...
				window.pageExt.form.onDatePickerChanged && window.pageExt.form.onDatePickerChanged("reviewDate",value, date, endDate);
			}
		});
		//渲染 methodId 下拉字段
		fox.renderSelectBox({
			el: "methodId",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("回访形式",'','cmp:form'),
			filterable: false,
			layVerify: 'required',
			layVerType: 'msg',
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("methodId",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					if(!data[i]) continue;
					if(window.pageExt.form.selectBoxDataTransform) {
						opts.push(window.pageExt.form.selectBoxDataTransform("methodId",{data:data[i],name:data[i].label,value:data[i].code,selected:(defaultValues.indexOf(data[i].code)!=-1 || defaultIndexs.indexOf(""+i)!=-1)},data[i],data,i));
					} else {
		...
		//渲染 satisfactionId 下拉字段
		fox.renderSelectBox({
			el: "satisfactionId",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("客户满意度",'','cmp:form'),
			filterable: false,
			layVerify: 'required',
			layVerType: 'msg',
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("satisfactionId",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					if(!data[i]) continue;
					if(window.pageExt.form.selectBoxDataTransform) {
						opts.push(window.pageExt.form.selectBoxDataTransform("satisfactionId",{data:data[i],name:data[i].label,value:data[i].code,selected:(defaultValues.indexOf(data[i].code)!=-1 || defaultIndexs.indexOf(""+i)!=-1)},data[i],data,i));
					} else {
						opts.push({data:data[i],name:data[i].label,value:data[i].code,selected:(defaultValues.indexOf(data[i].code)!=-1 || defaultIndexs.indexOf(""+i)!=-1)});
					}
				}
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('oa-crm-customer-review-form-data');
		}
		...
		if (hasData) {
			...
			if(formData["reviewDate"]) {
				$("#reviewDate").val(fox.dateFormat(formData["reviewDate"],"yyyy-MM-dd"));
			}


			//设置  回访形式 设置下拉框勾选
			fox.setSelectValue4QueryApi("#methodId",formData.reviewMethod);
			//设置  客户满意度 设置下拉框勾选
			fox.setSelectValue4QueryApi("#satisfactionId",formData.satisfaction);
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('oa-crm-customer-review-form-data');
		}
		return rawFormData;
	}
	...
	function getFormData() {
		...
		//获取 回访形式 下拉框的值
		data["methodId"]=fox.getSelectedValue("methodId",false);
		//获取 客户满意度 下拉框的值
		data["satisfactionId"]=fox.getSelectedValue("satisfactionId",false);
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('oa-crm-customer-review-form-data-win');
				}
    ...
    function bindButtonEvent() {
		...
		// 请选择人员对话框
		$("#reviewUserId-button").click(function(){
				var reviewUserIdDialogOptions={
				field:"reviewUserId",
				formData:getFormData(),
				inputEl:$("#reviewUserId"),
				buttonEl:$(this),
				single:true,
				autoWidth:false,
			...
			fox.chooseEmployee(reviewUserIdDialogOptions);
		});
		// 请选择人员对话框
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('oa-crm-customer-review-form-data-win',this); });
