...
function ListPage() {
    ...
    //模块基础路径
    const moduleURL="/service-oa/oa-netdisk-virtual-fd";
    const queryURL=moduleURL+'/query-paged-list';
    const deleteURL=moduleURL+'/delete';
    const batchDeleteURL=moduleURL+'/batch-delete';
        ...
        function renderTableInternal() {
            ...
            var tableConfig={
                ...
                cols: [[
                    ...
                    ,{ field: 'id', align:"left",fixed:false,  hide:true, sort: false  , title: fox.translate('主键') , templet: function (d) { return templet('id',d.id,d);}  }
                    ,{ field: 'fdName',event:"fdNameEvent",align:"left",fixed:false,  hide:false, sort: false  , title: fox.translate('名称') , templet: function (d) { return templet('fdName',d.fdName,d);}  }
                    ,{ field: 'fdSize', align:"left",fixed:false,  hide:false, sort: false  , title: fox.translate('大小') , templet: function (d) { return templet('fdSize',d.fdSize,d);}  }
                    ,{ field: 'createTime', align:"right", fixed:false, hide:false, sort: false   ,title: fox.translate('创建时间') ,templet: function (d) { return templet('createTime',fox.dateFormat(d.createTime,"yyyy-MM-dd HH:mm:ss"),d); }  }
    ...
    function refreshTableData(sortField,sortType,reset) {
        ...
        var value = {};
       // value.fil={ inputType:"button",value: $("#fileId").val()};
        var ps={searchField:"$composite"};
        if(window.pageExt.list.beforeQuery){
            if(!window.pageExt.list.beforeQuery(value,ps,"refresh")) return;
        }
        ps.fdName=$("#fdName").val();
    ...
    function bindButtonEvent() {
        ...
        table.on('toolbar(data-table)', function(obj){
            ...
            switch(obj.event){
                ...
                case 'refresh-data':
                    ...
                    break;
                case 'uploadFile':
                    var queryString="?fdId="+CURRENT_NODE.fdId;
                    admin.popupCenter({
                        title: "上传文件",
                        resize: false,
                        offset: [20,null],
                ...
                case 'createFolder':
                    layer.prompt({title:'请输入文件夹名称'},function(val,index){
                        ...
                        admin.post("/service-oa/oa-netdisk-folder/create-folder", { parentId:CURRENT_NODE.fdId,name:val }, function (data) {
                            if (data.success) {
                                layer.close(index);
                                fox.showMessage(data);
                                refreshTableData();
        ...
        function openCreateFrom() {
            //设置新增是初始化数据
            var data={};
            admin.putTempData('oa-netdisk-my-favorite-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            var ids=getCheckedList("id");
            if(ids.length==0) {
                top.layer.msg(fox.translate('请选择需要删除的'+'我的收藏'+"!"));
                return;
            }
            //调用批量删除接口
            top.layer.confirm(fox.translate('确定删除已选中的'+'我的收藏'+'吗？'), function (i) {
                top.layer.close(i);
                admin.post(batchDeleteURL, { ids: ids }, function (data) {
                    if (data.success) {
    ...
    function bindRowOperationEvent() {
        ...
        table.on('tool(data-table)', function (obj) {
            ...
            admin.putTempData('oa-netdisk-my-favorite-form-data-form-action', "",true);
            if (layEvent === 'edit') { // 修改
                top.layer.load(2);
                top.layer.load(2);
                admin.post(getByIdURL, { id : data.id }, function (data) {
                    ...
                    if(data.success) {
                        admin.putTempData('oa-netdisk-my-favorite-form-data-form-action', "edit",true);
                ...
                fox.submit(downloadUrl,{fdId:data.id,fdType:"file"});
            }
                ...
                top.layer.confirm(fox.translate('确定删除此'+'我的收藏'+'吗？'), function (i) {
                    top.layer.close(i);
               ...
               // , {"id":"fav","title":"收藏"}
                    ...
                    console.log("ops-more",data);
                    if(data.fdType=="folder"){
                        items = [
                            {"id":"rename","title":"重命名"},
                            {"id":"del","title":"删除"}
                        ];
                    }else if(data.fdType=="file"){
                        items = [
                            {"id":"share","title":"共享"},
                            {"id":"rename","title":"重命名"},
                            {"id":"del","title":"删除"}
    ...
    function showEditForm(data) {
        ...
        var action=admin.getTempData('oa-netdisk-my-favorite-form-data-form-action');
        var queryString="";
        if(data && data.id) queryString='id=' + data.id;
        if(window.pageExt.list.makeFormQueryString) {
            queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
        }
        admin.putTempData('oa-netdisk-my-favorite-form-data', data);
        var area=admin.getTempData('oa-netdisk-my-favorite-form-area');
        var height= (area && area.height) ? area.height : ($(window).height()*0.6);
        var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
        var title = fox.translate('我的收藏');
        ...
        admin.popupCenter({
            ...
            type: 2,
            id:"oa-netdisk-my-favorite-form-data-win",
            content: '/business/oa/netdisk_my_favorite/netdisk_my_favorite_form.html' + (queryString?("?"+queryString):""),
            finish: function () {
                if(action=="create") {
                    refreshTableData();
                }
