...
function ListPage() {
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'customerId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('客户'), templet: function (d) { return templet('customerId' ,fox.joinLabel(d.crmCustomer,"name",',','','customerId'),d);}}
					,{ field: 'name', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('名称') , templet: function (d) { return templet('name',d.name,d);}  }
					,{ field: 'reviewUserId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('回访人') , templet: function (d) { return templet('reviewUserId',fox.getProperty(d,["reviewUser","name"],0,'','reviewUserId'),d);} }
					,{ field: 'reviewDate', align:"right", fixed:false, hide:false, sort: true   ,title: fox.translate('回访时间') ,templet: function (d) { return templet('reviewDate',fox.dateFormat(d.reviewDate,"yyyy-MM-dd"),d); }  }
					,{ field: 'methodId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('回访形式'), templet: function (d) { return templet('methodId' ,fox.joinLabel(d.reviewMethod,"label",',','','methodId'),d);}}
					,{ field: 'satisfactionId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('客户满意度'), templet: function (d) { return templet('satisfactionId' ,fox.joinLabel(d.satisfaction,"label",',','','satisfactionId'),d);}}
	...
	function refreshTableData(sortField,sortType,reset) {
		...
		value.reviewDate={ inputType:"date_input", begin: $("#reviewDate-begin").val(), end: $("#reviewDate-end").val() ,matchType:"auto" };
	...
	function initSearchFields() {
		...
		laydate.render({
			elem: '#reviewDate-begin',
			trigger:"click",
			done: function(value, date, endDate) {
				setTimeout(function () {
					window.pageExt.list.onDatePickerChanged && window.pageExt.list.onDatePickerChanged("reviewDate",value, date, endDate);
				},1);
			}
		});
		laydate.render({
	...
	function bindButtonEvent() {
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'回访'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'回访'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('oa-crm-customer-review-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				top.layer.load(2);
				top.layer.load(2);
				admin.post(getByIdURL, { id : data.id }, function (data) {
					...
					if(data.success) {
						admin.putTempData('oa-crm-customer-review-form-data-form-action', "edit",true);
				...
				admin.post(getByIdURL, { id : data.id }, function (data) {
					top.layer.closeAll('loading');
					if(data.success) {
						admin.putTempData('oa-crm-customer-review-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
				...
				top.layer.confirm(fox.translate('确定删除此'+'回访'+'吗？'), function (i) {
					top.layer.close(i);
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('oa-crm-customer-review-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('oa-crm-customer-review-form-data', data);
		var area=admin.getTempData('oa-crm-customer-review-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('回访');
