...
layui.define(['form', 'table', 'util', 'settings', 'admin', 'upload','foxnic','xmSelect','laydate','foxnicUpload','dropdown'],function () {
    ...
    var formAction=admin.getTempData('oa-crm-customer-review-form-data-form-action');
    //列表页的扩展
    var list={
        /**
         * 列表页初始化前调用
        ...
        makeFormQueryString:function(data,queryString,action) {
            ...
            if(data&&data.id){
                queryString="id="+data.id+"&customerId="+CUSTOMER_ID
            }else{
    ...
    var form={
        ...
        beforeInit:function (action,data) {
            ...
            if(formAction=="create"){
                //默认填充报修人员
                if($("#reviewUserId")){
                    $("#reviewUserId").val(CUR_EMP_ID);
                }
                if($("#reviewUserId-button")){
                    var html="<i class=\"layui-icon layui-icon-search\"></i><span default-label=\"回访人\">"+CUR_USER_NAME+"</span>"
                    $("#reviewUserId-button").html(html);
                }
