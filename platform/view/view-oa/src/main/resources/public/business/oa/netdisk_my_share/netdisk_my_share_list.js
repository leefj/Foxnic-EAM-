...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-oa/oa-netdisk-my-share";
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'fileUrl', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('链接') , templet: function (d) { return templet('fileUrl',d.fileUrl,d);}  }
			//		,{ field: 'expirationTime', align:"right", fixed:false, hide:false, sort: true   ,title: fox.translate('过期时间') ,templet: function (d) { return templet('expirationTime',fox.dateFormat(d.expirationTime,"yyyy-MM-dd HH:mm:ss"),d); }  }
					,{ field: 'expirationMethod', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('过期方式'), templet:function (d){ return templet('expirationMethod',fox.getEnumText(RADIO_EXPIRATIONMETHOD_DATA,d.expirationMethod,'','expirationMethod'),d);}}
	...
	function refreshTableData(sortField,sortType,reset) {
		...
		value.fileUrl={ inputType:"button",value: $("#fileUrl").val()};
		value.expirationTime={ inputType:"date_input", value: $("#expirationTime").val() ,matchType:"auto"};
		value.expirationMethod={ inputType:"radio_box", value: getSelectedValue("#expirationMethod","value"), label:getSelectedValue("#expirationMethod","nameStr") };
	...
	function initSearchFields() {
		...
		laydate.render({
			...
			trigger:"click",
				...
				setTimeout(function () {
					window.pageExt.list.onDatePickerChanged && window.pageExt.list.onDatePickerChanged("expirationTime",value, date, endDate);
				},1);
			}
		});
		//渲染 expirationMethod 搜索框
		fox.renderSelectBox({
			el: "expirationMethod",
			size: "small",
			radio: true,
			on: function(data){
				setTimeout(function () {
					window.pageExt.list.onSelectBoxChanged && window.pageExt.list.onSelectBoxChanged("expirationMethod",data.arr,data.change,data.isAdd);
				},1);
			},
			...
			transform:function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					if(window.pageExt.list.selectBoxDataTransform) {
						opts.push(window.pageExt.list.selectBoxDataTransform("expirationMethod",{data:data[i],name:data[i].text,value:data[i].code},data[i],data,i));
					} else {
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('oa-netdisk-my-share-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('oa-netdisk-my-share-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'我的分享'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'我的分享'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('oa-netdisk-my-share-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				top.layer.load(2);
				top.layer.load(2);
				admin.post(getByIdURL, { id : data.id }, function (data) {
					...
					if(data.success) {
						admin.putTempData('oa-netdisk-my-share-form-data-form-action', "edit",true);
				...
				var queryString="?shareId="+data.id+"&fdId="+data.fileId;
				var title = fox.translate('分享成员');
				admin.popupCenter({
					title: title,
					resize: false,
					offset: [20,null],
				...
				top.layer.confirm(fox.translate('确定取消此'+'我的分享'+'吗？'), function (i) {
					top.layer.close(i);
					...
					},{delayLoading:100, elms:[$(".ops-cancel-button[data-id='"+data.id+"']")]});
				});
			}
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('oa-netdisk-my-share-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('oa-netdisk-my-share-form-data', data);
		var area=admin.getTempData('oa-netdisk-my-share-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('我的分享');
		...
		admin.popupCenter({
			...
			type: 2,
			id:"oa-netdisk-my-share-form-data-win",
			content: '/business/oa/netdisk_my_share/netdisk_my_share_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
