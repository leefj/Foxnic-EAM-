...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-eam/iot-virt-equipment";
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'ownerId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('所属产品'), templet: function (d) { return templet('ownerId' ,fox.joinLabel(d.iotProduct,"name",',','','ownerId'),d);}}
					...
					,{ field: 'triggerType', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('触发方式'), templet:function (d){ return templet('triggerType',fox.getEnumText(RADIO_TRIGGERTYPE_DATA,d.triggerType,'','triggerType'),d);}}
					...
					,{ field: 'crontabValue', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('crontab表达式') , templet: function (d) { return templet('crontabValue',d.crontabValue,d);}  }
	...
	function refreshTableData(sortField,sortType,reset) {
		...
		value.ownerId={ inputType:"select_box", value: getSelectedValue("#ownerId","value") ,fillBy:["iotProduct"]  , label:getSelectedValue("#ownerId","nameStr") };
		value.type={ inputType:"radio_box", value: getSelectedValue("#type","value"), label:getSelectedValue("#type","nameStr") };
		value.triggerType={ inputType:"radio_box", value: getSelectedValue("#triggerType","value"), label:getSelectedValue("#triggerType","nameStr") };
	...
	function initSearchFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.list.onSelectBoxChanged && window.pageExt.list.onSelectBoxChanged("ownerId",data.arr,data.change,data.isAdd);
				},1);
			},
			paging: true,
			pageRemote: true,
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					if(window.pageExt.list.selectBoxDataTransform) {
						opts.push(window.pageExt.list.selectBoxDataTransform("ownerId",{data:data[i],name:data[i].name,value:data[i].id},data[i],data,i));
					} else {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.list.onSelectBoxChanged && window.pageExt.list.onSelectBoxChanged("triggerType",data.arr,data.change,data.isAdd);
				},1);
			},
			//toolbar: {show:true,showIcon:true,list:["CLEAR","REVERSE"]},
			transform:function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					if(window.pageExt.list.selectBoxDataTransform) {
						opts.push(window.pageExt.list.selectBoxDataTransform("triggerType",{data:data[i],name:data[i].text,value:data[i].code},data[i],data,i));
					} else {
						opts.push({data:data[i],name:data[i].text,value:data[i].code});
					}
				}
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('iot-virt-equipment-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('iot-virt-equipment-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'虚拟设备'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'虚拟设备'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('iot-virt-equipment-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				top.layer.load(2);
				top.layer.load(2);
				admin.post(getByIdURL, { id : data.id }, function (data) {
					...
					if(data.success) {
						admin.putTempData('iot-virt-equipment-form-data-form-action', "edit",true);
				...
				admin.post(getByIdURL, { id : data.id }, function (data) {
					top.layer.closeAll('loading');
					if(data.success) {
						admin.putTempData('iot-virt-equipment-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
				...
				top.layer.confirm(fox.translate('确定删除此'+'虚拟设备'+'吗？'), function (i) {
					top.layer.close(i);
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('iot-virt-equipment-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('iot-virt-equipment-form-data', data);
		var area=admin.getTempData('iot-virt-equipment-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('虚拟设备');
		...
		admin.popupCenter({
			...
			area: ["80%",height+"px"],
			type: 2,
			id:"iot-virt-equipment-form-data-win",
			content: '/business/eam/virt_equipment/virt_equipment_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
