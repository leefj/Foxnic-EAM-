...
function ListPage() {
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('eam-asset-stock-goods-use-form-data', {});
					var defaultValue={};
					if(window.pageExt.list.getBpmViewConfig) {
						defaultValue=window.pageExt.list.getBpmViewConfig(obj.event);
					}
					admin.putTempData('eam-asset-stock-goods-use-form-data-form-action', "create",true);
					bpm.openProcessView(null,null,false,{"formDefinitionCode":"eam_asset_stock_goods_use"},refreshTableData,refreshRowData,"bill",defaultValue);
					break;
				case 'batch-del':
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('eam-asset-stock-goods-use-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				top.layer.load(2);
				bpm.getProcessInstanceByBill("eam_asset_stock_goods_use",{ id : data.id },function(p) {
					top.layer.closeAll('loading');
					if(p) {
						admin.putTempData('eam-asset-stock-goods-use-form-data-form-action', "edit",true);
						bpm.openProcessView(p.id,null,false,{"formDefinitionCode":"eam_asset_stock_goods_use"},refreshTableData,refreshRowData,"bill");
					} else {
				...
				admin.post(getByIdURL, { id : data.id }, function (data) {
					top.layer.closeAll('loading');
					if(data.success) {
						admin.putTempData('eam-asset-stock-goods-use-form-data-form-action', "view",true);
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('eam-asset-stock-goods-use-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('eam-asset-stock-goods-use-form-data', data);
		...
		admin.popupCenter({
			...
			area: ["80%",height+"px"],
			type: 2,
			id:"eam-asset-stock-goods-use-form-data-win",
			content: '/business/eam/asset_stock_goods_use/asset_stock_goods_use_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
