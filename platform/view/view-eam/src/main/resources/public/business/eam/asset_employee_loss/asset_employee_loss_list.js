...
function ListPage() {
	...
	function bindButtonEvent() {
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'资产报失'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'资产报失'+'吗？'), function (i) {
                top.layer.close(i);
		...
		table.on('tool(data-table)', function (obj) {
			...
			if (layEvent === 'edit') { // 修改
				...
				bpm.getProcessInstanceByBill("eam_asset_employee_loss",{ id : data.id },function(p) {
					...
					if(p) {
						admin.putTempData('eam-asset-employee-loss-form-data-form-action', "edit",true);
