...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('eam-work-order-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('eam-work-order-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'eam-work-order-form-data-win');
			if(area==null) return;
			admin.putTempData('eam-work-order-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			el: "status",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("工单状态",'','cmp:form'),
			filterable: false,
			on: function(data){
				setTimeout(function () {
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('eam-work-order-form-data');
		}
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('eam-work-order-form-data');
		}
		return rawFormData;
	}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('eam-work-order-form-data-win');
				}
    ...
    function bindButtonEvent() {
		...
		// 请选择人员对话框
		$("#rcdUserId-button").click(function(){
				var rcdUserIdDialogOptions={
				field:"rcdUserId",
			...
			fox.chooseEmployee(rcdUserIdDialogOptions);
		});

	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('eam-work-order-form-data-win',this); });
