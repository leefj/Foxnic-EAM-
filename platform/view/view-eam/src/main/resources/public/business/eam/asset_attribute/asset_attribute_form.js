...
function FormPage() {
	...
	this.init=function(layui) {
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('eam-asset-attribute-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'eam-asset-attribute-form-data-win');
			if(area==null) return;
	...
	function renderFormFields() {
		...
		form.on('radio(valueType)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=valueType]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("valueType",data,checked);
		});
		...
		form.on('radio(requiredModify)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=requiredModify]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("requiredModify",data,checked);
		});
		form.on('radio(componentType)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=componentType]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("componentType",data,checked);
		});
		//渲染 owner 下拉字段
		fox.renderSelectBox({
			el: "owner",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("归属",'','cmp:form'),
			filterable: false,
			layVerify: 'required',
			layVerType: 'msg',
			on: function(data){
			...
			transform:function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					if(window.pageExt.form.selectBoxDataTransform) {
						opts.push(window.pageExt.form.selectBoxDataTransform("owner",{data:data[i],name:data[i].text,value:data[i].code,selected:(defaultValues.indexOf(data[i].code)!=-1 || defaultIndexs.indexOf(""+i)!=-1)},data[i],data,i));
					} else {
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('eam-asset-attribute-form-data');
		}
		return rawFormData;
	}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('eam-asset-attribute-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('eam-asset-attribute-form-data-win',this); });
