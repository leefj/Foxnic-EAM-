...
function FormPage() {
	...
	function renderFormFields() {
		...
		form.on('radio(assetCheck)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=assetCheck]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("assetCheck",data,checked);
		});
	    //渲染图片字段
		foxup.render({
			el:"attach",
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('eam-purchase-apply-form-data');
		}
		return rawFormData;
	}
    ...
    function bindButtonEvent() {
		...
		// 请选择人员对话框
		$("#purchaseUserId-button").click(function(){
				var purchaseUserIdDialogOptions={
				field:"purchaseUserId",
				formData:getFormData(),
				inputEl:$("#purchaseUserId"),
				buttonEl:$(this),
				single:true,
				autoWidth:false,
				//限制浏览的范围，指定根节点 id 或 code ，优先匹配ID
			...
			fox.chooseEmployee(purchaseUserIdDialogOptions);
		});
