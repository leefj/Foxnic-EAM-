    ...
    function renderTable() {
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				url: '/service-eam/eam-goods-stock-related/query-paged-list-by-selected',
				height: 'full-'+(h+40),
				limit: 5,
				where: ps,
				cols: [[
					...
					,{ field: 'goodsStatus', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('档案状态'), templet:function (d){ return templet('goodsStatus',fox.getEnumText(SELECT_GOODSSTATUS_DATA,d.goodsStatus),d);}}
					,{ field: 'categoryId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('档案分类'), templet: function (d) { return templet('categoryId' ,fox.joinLabel(d.category,"name"),d);}}
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					...
					if(PAGE_TYPE=="create"){
						ownerTmpId=SELECTED_CODE;
					}else{
						ownerTmpId=OWNER_ID;
					}
					var operType="eam_asset_stock_goods_sub_select";
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			if (layEvent === 'edit') { // 修改
				...
				var delpos={};
				delpos.id=data.id;
				delpos.ownerId=OWNER_ID;
				delpos.selectCode=SELECTED_CODE;
				top.layer.confirm(fox.translate('确定删除此')+fox.translate('库存物品')+fox.translate('吗？'), function (i) {
					top.layer.close(i);
					top.layer.load(2);
					admin.request("/service-eam/eam-goods-stock-related/select-delete-by-id", delpos, function (data) {
