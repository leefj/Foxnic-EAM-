...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-eam/eam-work-order";
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'content', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('问题') , templet: function (d) { return templet('content',d.content,d);}  }
					...
					,{ field: 'rcdUserId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('操作人') , templet: function (d) { return templet('rcdUserId',fox.getProperty(d,["originator","name"],0,'','rcdUserId'),d);} }
					,{ field: 'rcdTime', align:"right", fixed:false, hide:false, sort: true   ,title: fox.translate('操作时间') ,templet: function (d) { return templet('rcdTime',fox.dateFormat(d.rcdTime,"yyyy-MM-dd"),d); }  }
					,{ field: 'lastTime', align:"right", fixed:false, hide:false, sort: true   ,title: fox.translate('回复时间') ,templet: function (d) { return templet('lastTime',fox.dateFormat(d.lastTime,"yyyy-MM-dd HH:mm:ss"),d); }  }
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('eam-work-order-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('eam-work-order-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'问题工单'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'问题工单'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('eam-work-order-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				top.layer.load(2);
				top.layer.load(2);
				admin.post(getByIdURL, { id : data.id }, function (data) {
					...
					if(data.success) {
						admin.putTempData('eam-work-order-form-data-form-action', "edit",true);
				...
				admin.post(getByIdURL, { id : data.id }, function (data) {
					top.layer.closeAll('loading');
					if(data.success) {
						admin.putTempData('eam-work-order-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
				...
				top.layer.confirm(fox.translate('确定删除此'+'问题工单'+'吗？'), function (i) {
					top.layer.close(i);
			...
			else if (layEvent === 'process-rcd') { // 查看
				window.pageExt.list.processRcd(data,this);
			}
			else if (layEvent === 'cancel-rcd') { // 取消
				window.pageExt.list.cancelRcd(data,this);
			}
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('eam-work-order-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('eam-work-order-form-data', data);
		var area=admin.getTempData('eam-work-order-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('问题工单');
		...
		admin.popupCenter({
			...
			area: ["75%",height+"px"],
			type: 2,
			id:"eam-work-order-form-data-win",
			content: '/business/eam/work_order/work_order_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
