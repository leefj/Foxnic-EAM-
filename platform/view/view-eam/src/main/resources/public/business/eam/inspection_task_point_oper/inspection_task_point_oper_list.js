...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-eam/eam-inspection-task-point-oper";
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'isPointStatus', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('巡检状态'), templet:function (d){ return templet('isPointStatus',fox.getEnumText(RADIO_ISPOINTSTATUS_DATA,d.isPointStatus,'','isPointStatus'),d);}}
					...
					,{ field: 'isActionLabel', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('处理动作'), templet:function (d){ return templet('isActionLabel',fox.getEnumText(RADIO_ISACTIONLABEL_DATA,d.isActionLabel,'','isActionLabel'),d);}}
					...
					,{ field: 'isInspectorId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('巡检人'), templet:function (d){ return templet('isInspectorId',fox.getEnumText(RADIO_ISINSPECTORID_DATA,d.isInspectorId,'','isInspectorId'),d);}}
					,{ field: 'inspectorId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('巡检人') , templet: function (d) { return templet('inspectorId',fox.getProperty(d,["inspector","name"],0,'','inspectorId'),d);} }
					,{ field: 'isContent', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('巡检结果'), templet:function (d){ return templet('isContent',fox.getEnumText(RADIO_ISCONTENT_DATA,d.isContent,'','isContent'),d);}}
					...
					,{ field: 'operUserId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('操作人') , templet: function (d) { return templet('operUserId',fox.getProperty(d,["operUser","name"],0,'','operUserId'),d);} }
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('eam-inspection-task-point-oper-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('eam-inspection-task-point-oper-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'巡检点批量操作'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'巡检点批量操作'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('eam-inspection-task-point-oper-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				top.layer.load(2);
				top.layer.load(2);
				admin.post(getByIdURL, { id : data.id }, function (data) {
					...
					if(data.success) {
						admin.putTempData('eam-inspection-task-point-oper-form-data-form-action', "edit",true);
				...
				admin.post(getByIdURL, { id : data.id }, function (data) {
					top.layer.closeAll('loading');
					if(data.success) {
						admin.putTempData('eam-inspection-task-point-oper-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
				...
				top.layer.confirm(fox.translate('确定删除此'+'巡检点批量操作'+'吗？'), function (i) {
					top.layer.close(i);
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('eam-inspection-task-point-oper-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('eam-inspection-task-point-oper-form-data', data);
		var area=admin.getTempData('eam-inspection-task-point-oper-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('巡检点批量操作');
		...
		admin.popupCenter({
			...
			area: ["80%",height+"px"],
			type: 2,
			id:"eam-inspection-task-point-oper-form-data-win",
			content: '/business/eam/inspection_task_point_oper/inspection_task_point_oper_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
