...
function ListPage() {
	...
	function refreshTableData(sortField,sortType,reset) {
		...
		var value = {};
		value.owner={ inputType:"radio_box", value: getSelectedValue("#owner","value"), label:getSelectedValue("#owner","nameStr") };
			...
			transform:function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					if(window.pageExt.list.selectBoxDataTransform) {
						opts.push(window.pageExt.list.selectBoxDataTransform("owner",{data:data[i],name:data[i].text,value:data[i].code},data[i],data,i));
					} else {
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					...
					var queryString="?ownerId="+OWNER_ID+"&selectCode="+SELECT_CODE;
				...
				//	var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
					...
					admin.popupCenter({
						...
						area: ["80%","85%"],
						type: 2,
						id:"eam-check-item-select-list-win",
						content: '/business/eam/check_item/check_item_select_list.html' +queryString,
						finish: function () {
							refreshTableData(null,null,true);
						}
					});
		...
		function openCreateFrom() {
			//设置新增是初始化数据
			var data={};
			admin.putTempData('eam-check-item-form-data-form-action', "create",true);
			showEditForm(data);
		};
		...
		function batchDelete(selected) {
			...
			if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'检查项'+"!"));
				return;
			}
			//调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'检查项'+'吗？'), function (i) {
				top.layer.close(i);
	...
	function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('eam-check-item-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				top.layer.load(2);
				top.layer.load(2);
				admin.post(getByIdURL, { id : data.id }, function (data) {
					...
					if(data.success) {
						admin.putTempData('eam-check-item-form-data-form-action', "edit",true);
				...
				admin.post(getByIdURL, { id : data.id }, function (data) {
					top.layer.closeAll('loading');
					if(data.success) {
						admin.putTempData('eam-check-item-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
				...
				top.layer.confirm(fox.translate('确定删除此'+'检查项'+'吗？'), function (i) {
					top.layer.close(i);
					admin.post(moduleURL+"/select-delete-by-id", {ownerId:OWNER_ID, id : data.id,selectCode:SELECT_CODE }, function (data) {
						top.layer.closeAll('loading');
						if (data.success) {
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('eam-check-item-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('eam-check-item-form-data', data);
		var area=admin.getTempData('eam-check-item-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('检查项');
		...
		admin.popupCenter({
			...
			area: ["80%",height+"px"],
			type: 2,
			id:"eam-check-item-form-data-win",
			content: '/business/eam/check_item/check_item_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
