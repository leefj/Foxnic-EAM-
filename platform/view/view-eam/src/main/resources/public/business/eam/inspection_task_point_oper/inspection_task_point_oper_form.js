...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('eam-inspection-task-point-oper-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('eam-inspection-task-point-oper-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'eam-inspection-task-point-oper-form-data-win');
			if(area==null) return;
			admin.putTempData('eam-inspection-task-point-oper-form-area', area);
	...
	function renderFormFields() {
		...
		form.on('radio(isPointStatus)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=isPointStatus]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("isPointStatus",data,checked);
		});
		...
		form.on('radio(isActionLabel)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=isActionLabel]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("isActionLabel",data,checked);
		});
		//渲染 actionLabel 下拉字段
		fox.renderSelectBox({
			el: "actionLabel",
		...
		form.on('radio(isInspectorId)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=isInspectorId]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("isInspectorId",data,checked);
		});
		form.on('radio(isContent)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=isContent]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("isContent",data,checked);
		});
		laydate.render({
			elem: '#operTime',
			type:"date",
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('eam-inspection-task-point-oper-form-data');
		}
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('eam-inspection-task-point-oper-form-data');
		}
		return rawFormData;
	}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('eam-inspection-task-point-oper-form-data-win');
				}
    ...
    function bindButtonEvent() {
		...
		// 请选择人员对话框
		$("#inspectorId-button").click(function(){
				var inspectorIdDialogOptions={
				field:"inspectorId",
			...
			fox.chooseEmployee(inspectorIdDialogOptions);
		});
		// 请选择人员对话框
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('eam-inspection-task-point-oper-form-data-win',this); });
