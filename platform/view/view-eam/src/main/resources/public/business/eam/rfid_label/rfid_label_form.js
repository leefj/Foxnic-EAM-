...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('eam-rfid-label-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('eam-rfid-label-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'eam-rfid-label-form-data-win');
			if(area==null) return;
			admin.putTempData('eam-rfid-label-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			el: "assetId",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("资产",'','cmp:form'),
			filterable: true,
			paging: true,
			pageRemote: true,
			layVerify: 'required',
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('eam-rfid-label-form-data');
		}
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('eam-rfid-label-form-data');
		}
		return rawFormData;
	}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('eam-rfid-label-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('eam-rfid-label-form-data-win',this); });
