...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-eam/eam-goods-stock-usage";
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'billCode', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('单据编码') , templet: function (d) { return templet('billCode',d.billCode,d);}  }
					,{ field: 'label', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('操作标签') , templet: function (d) { return templet('label',d.label,d);}  }
					...
					,{ field: 'operNumber', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('数量') , templet: function (d) { return templet('operNumber',d.operNumber,d);}  }
					,{ field: 'operUserName', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('操作人员') , templet: function (d) { return templet('operUserName',d.operUserName,d);}  }
	...
	function refreshTableData(sortField,sortType,reset) {
		...
		var value = {};
		value.billCode={ inputType:"button",value: $("#billCode").val() ,fuzzy: true,splitValue:false,valuePrefix:"",valueSuffix:"" };
		...
		value.operUserName={ inputType:"button",value: $("#operUserName").val() ,fuzzy: true,splitValue:false,valuePrefix:"",valueSuffix:"" };
		var ps={searchField:"$composite"};
		if(window.pageExt.list.beforeQuery){
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('eam-goods-stock-usage-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('eam-goods-stock-usage-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'使用情况'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'使用情况'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('eam-goods-stock-usage-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				top.layer.load(2);
				top.layer.load(2);
				admin.post(getByIdURL, { id : data.id }, function (data) {
					...
					if(data.success) {
						admin.putTempData('eam-goods-stock-usage-form-data-form-action', "edit",true);
				...
				admin.post(getByIdURL, { id : data.id }, function (data) {
					top.layer.closeAll('loading');
					if(data.success) {
						admin.putTempData('eam-goods-stock-usage-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
				...
				top.layer.confirm(fox.translate('确定删除此'+'使用情况'+'吗？'), function (i) {
					top.layer.close(i);
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('eam-goods-stock-usage-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('eam-goods-stock-usage-form-data', data);
		var area=admin.getTempData('eam-goods-stock-usage-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('使用情况');
		...
		admin.popupCenter({
			...
			area: ["80%",height+"px"],
			type: 2,
			id:"eam-goods-stock-usage-form-data-win",
			content: '/business/eam/goods_stock_usage/goods_stock_usage_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
