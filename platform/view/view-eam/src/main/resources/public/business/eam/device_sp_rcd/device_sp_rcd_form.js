...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('eam-device-sp-rcd-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('eam-device-sp-rcd-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'eam-device-sp-rcd-form-data-win');
			if(area==null) return;
			admin.putTempData('eam-device-sp-rcd-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			el: "spId",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("备件",'','cmp:form'),
			filterable: true,
			paging: true,
			pageRemote: true,
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("spId",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					if(window.pageExt.form.selectBoxDataTransform) {
						opts.push(window.pageExt.form.selectBoxDataTransform("spId",{data:data[i],name:data[i].name,value:data[i].id,selected:(defaultValues.indexOf(data[i].id)!=-1 || defaultIndexs.indexOf(""+i)!=-1)},data[i],data,i));
					} else {
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('eam-device-sp-rcd-form-data');
		}
		...
		if (hasData) {
			...
			//设置  备件 设置下拉框勾选
			fox.setSelectValue4QueryApi("#spId",formData.deviceSp);
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('eam-device-sp-rcd-form-data');
		}
		return rawFormData;
	}
	...
	function getFormData() {
		...
		//获取 备件 下拉框的值
		data["spId"]=fox.getSelectedValue("spId",false);
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('eam-device-sp-rcd-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('eam-device-sp-rcd-form-data-win',this); });
