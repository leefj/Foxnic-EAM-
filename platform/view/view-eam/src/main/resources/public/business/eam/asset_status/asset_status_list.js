...
function ListPage() {
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				...
				case 'batch-del':
					...
					break;
				case 'tool-status-rule':
					window.pageExt.list.statusRule && window.pageExt.list.statusRule(selected,obj);
					break;
				case 'refresh-data':
					refreshTableData();
					break;
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'资产状态'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'资产状态'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			if (layEvent === 'edit') { // 修改
				...
				top.layer.confirm(fox.translate('确定删除此'+'资产状态'+'吗？'), function (i) {
					top.layer.close(i);
