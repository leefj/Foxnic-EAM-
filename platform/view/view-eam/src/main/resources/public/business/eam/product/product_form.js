...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('iot-product-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('iot-product-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'iot-product-form-data-win');
			if(area==null) return;
			admin.putTempData('iot-product-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			el: "categoryId",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("品类",'','cmp:form'),
			filterable: false,
			layVerify: 'required',
			layVerType: 'msg',
			on: function(data){
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					if(window.pageExt.form.selectBoxDataTransform) {
						opts.push(window.pageExt.form.selectBoxDataTransform("categoryId",{data:data[i],name:data[i].label,value:data[i].code,selected:(defaultValues.indexOf(data[i].code)!=-1 || defaultIndexs.indexOf(""+i)!=-1)},data[i],data,i));
					} else {
		...
		form.on('radio(productType)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=productType]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("productType",data,checked);
		});
		form.on('radio(productTransparent)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=productTransparent]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("productTransparent",data,checked);
		});
		form.on('radio(locationStatus)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=locationStatus]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("locationStatus",data,checked);
		});
		form.on('radio(locationMethod)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=locationMethod]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("locationMethod",data,checked);
		});
	    //渲染图片字段
		foxup.render({
			el:"imageId",
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('iot-product-form-data');
		}
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('iot-product-form-data');
		}
		return rawFormData;
	}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('iot-product-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('iot-product-form-data-win',this); });
