...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('eam-stock-import-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('eam-stock-import-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'eam-stock-import-form-data-win');
			if(area==null) return;
			admin.putTempData('eam-stock-import-form-area', area);
	...
	function renderFormFields() {
		...
		form.on('radio(importToType)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=importToType]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("importToType",data,checked);
		});
	}
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('eam-stock-import-form-data');
		}
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('eam-stock-import-form-data');
		}
		return rawFormData;
	}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('eam-stock-import-form-data-win');
				}
    ...
    function bindButtonEvent() {
		...
		$("#orderId-button").click(function(){
			window.pageExt.form.selectOrder && window.pageExt.form.selectOrder(getFormData(),$("#orderId"),$(this));
		});
		// 请选择公司对话框
		...
		$("#fill-button").click(function(){
			window.pageExt.form.fillData && window.pageExt.form.fillData(getFormData(),$("#fill"),$(this));
		});

	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('eam-stock-import-form-data-win',this); });
