...
function FormPage() {
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			el: "resultType",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("维修结果",'','cmp:form'),
			filterable: true,
			layVerify: 'required',
			layVerType: 'msg',
			on: function(data){
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					if(window.pageExt.form.selectBoxDataTransform) {
						opts.push(window.pageExt.form.selectBoxDataTransform("resultType",{data:data[i],name:data[i].label,value:data[i].code,selected:(defaultValues.indexOf(data[i].code)!=-1 || defaultIndexs.indexOf(""+i)!=-1)},data[i],data,i));
					} else {
		...
		fox.renderSelectBox({
			el: "categoryTplId",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("实际故障",'','cmp:form'),
			filterable: true,
			layVerify: 'required',
			layVerType: 'msg',
			on: function(data){
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('eam-repair-order-acceptance-form-data');
		}
		return rawFormData;
	}
