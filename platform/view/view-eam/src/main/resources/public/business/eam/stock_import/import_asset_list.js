...
function ListPage() {
	...
	function renderTable() {
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				url: '/service-eam/eam-asset/query-raw-paged-list',
				height: 'full-'+(h+28),
				limit: 50,
				where: ps,
				cols: [[
					...
					,{ field: 'name', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('资产名称') , templet: function (d) { return templet('name',d.name,d);}  }
	...
	function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			if (layEvent === 'edit') { // 修改
				...
				admin.request("/service-eam/eam-asset/get-by-id", { id : data.id }, function (data) {
					clearTimeout(task);
					layer.closeAll('loading');
					if(data.success) {
						admin.putTempData('eam-asset-form-data-form-action', "edit",true);
						showAssetEditForm(data.data);
					} else {
				...
				top.layer.confirm(fox.translate('确定删除此')+fox.translate('资产数据')+fox.translate('吗？'), function (i) {
					top.layer.close(i);
					admin.post("/service-eam/eam-asset/delete", { id : data.id }, function (data) {
	...
	function showAssetEditForm(data) {
		if(window.pageExt.list.beforeEdit) {
			var doNext=window.pageExt.list.beforeEdit(data);
			if(!doNext) return;
		}
		var action=admin.getTempData('eam-asset-form-data-form-action');
		var queryString="?ownerCode=asset&pageType=base&categoryCode=asset";
//		var queryString="";
		if(data && data.id){
