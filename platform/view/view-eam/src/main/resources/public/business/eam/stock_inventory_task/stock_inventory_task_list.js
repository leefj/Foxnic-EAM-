...
function ListPage() {
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'dataStatus', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('数据状态'), templet:function (d){ return templet('dataStatus',fox.getEnumText(SELECT_DATASTATUS_DATA,d.dataStatus,'','dataStatus'),d);}}
					...
					,{ field: 'directorId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('负责人') , templet: function (d) { return templet('directorId',fox.getProperty(d,["director","name"],0,'','directorId'),d);} }
					...
					,{ field: 'inventoryAssetCountByNotCounted', align:"",fixed:false,  hide:false, sort: false  , title: fox.translate('待盘数') , templet: function (d) { return templet('inventoryAssetCountByNotCounted',d.inventoryAssetCountByNotCounted,d);}  }
					,{ field: 'inventoryAssetCountByCounted', align:"",fixed:false,  hide:false, sort: false  , title: fox.translate('已盘数') , templet: function (d) { return templet('inventoryAssetCountByCounted',d.inventoryAssetCountByCounted,d);}  }
					,{ field: 'inventoryAssetCountByLoss', align:"",fixed:false,  hide:false, sort: false  , title: fox.translate('盘亏数') , templet: function (d) { return templet('inventoryAssetCountByLoss',d.inventoryAssetCountByLoss,d);}  }
					,{ field: 'inventoryAssetCountBySurplus', align:"",fixed:false,  hide:false, sort: false  , title: fox.translate('盘赢数') , templet: function (d) { return templet('inventoryAssetCountBySurplus',d.inventoryAssetCountBySurplus,d);}  }
					,{ field: 'inventoryAssetCountByException', align:"",fixed:false,  hide:false, sort: false  , title: fox.translate('异常数') , templet: function (d) { return templet('inventoryAssetCountByException',d.inventoryAssetCountByException,d);}  }
					,{ field: 'warehouseIds', align:"",fixed:false,  hide:false, sort: false  , title: fox.translate('所在仓库'), templet: function (d) { return templet('warehouseIds' ,fox.joinLabel(d.warehouseList,"warehouseName",',','','warehouseIds'),d);}}
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('eam-stock-inventory-task-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('eam-stock-inventory-task-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'盘点任务2'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'盘点任务2'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('eam-stock-inventory-task-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				top.layer.load(2);
				top.layer.load(2);
				admin.post(getByIdURL, { id : data.id }, function (data) {
					...
					if(data.success) {
						admin.putTempData('eam-stock-inventory-task-form-data-form-action', "edit",true);
				...
				admin.post(getByIdURL, { id : data.id }, function (data) {
					top.layer.closeAll('loading');
					if(data.success) {
						admin.putTempData('eam-stock-inventory-task-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
				...
				top.layer.confirm(fox.translate('确定删除此'+'盘点任务2'+'吗？'), function (i) {
					top.layer.close(i);
				...
				window.pageExt.list.inventoryDetail(data,this);
			}
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('eam-stock-inventory-task-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('eam-stock-inventory-task-form-data', data);
		var area=admin.getTempData('eam-stock-inventory-task-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('盘点任务2');
		...
		admin.popupCenter({
			...
			content: '/business/eam/stock_inventory_task/stock_inventory_task_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
