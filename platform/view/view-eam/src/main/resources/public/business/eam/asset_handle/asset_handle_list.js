...
function ListPage() {
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'type', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('处置类型'), templet: function (d) { return templet('type' ,fox.joinLabel(d.assetHandleType,"label",',','','type'),d);}}
					,{ field: 'planFinishDate', align:"right", fixed:false, hide:false, sort: true   ,title: fox.translate('处理日期') ,templet: function (d) { return templet('planFinishDate',fox.dateFormat(d.planFinishDate,"yyyy-MM-dd"),d); }  }
	...
	function refreshTableData(sortField,sortType,reset) {
		...
		value.type={ inputType:"select_box", value: getSelectedValue("#type","value") ,fillBy:["assetHandleType"]  , label:getSelectedValue("#type","nameStr") };
	...
	function initSearchFields() {
		...
		laydate.render({
			elem: '#planFinishDate-begin',
			trigger:"click",
			done: function(value, date, endDate) {
				setTimeout(function () {
					window.pageExt.list.onDatePickerChanged && window.pageExt.list.onDatePickerChanged("planFinishDate",value, date, endDate);
				},1);
			}
		});
		laydate.render({
			elem: '#planFinishDate-end',
			trigger:"click",
			done: function(value, date, endDate) {
				setTimeout(function () {
					window.pageExt.list.onDatePickerChanged && window.pageExt.list.onDatePickerChanged("planFinishDate",value, date, endDate);
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('eam-asset-handle-form-data', {});
					var defaultValue={};
					...
					bpm.openProcessView(null,null,false,{"formDefinitionCode":"eam_asset_handle"},refreshTableData,refreshRowData,"bill",defaultValue);
					break;
				case 'batch-del':
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'资产处置'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'资产处置'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			if (layEvent === 'edit') { // 修改
				top.layer.load(2);
				bpm.getProcessInstanceByBill("eam_asset_handle",{ id : data.id },function(p) {
					top.layer.closeAll('loading');
					if(p) {
						admin.putTempData('eam-asset-handle-form-data-form-action', "edit",true);
						bpm.openProcessView(p.id,null,false,{"formDefinitionCode":"eam_asset_handle"},refreshTableData,refreshRowData,"bill");
					} else {
	...
	function showEditForm(data) {
		...
		admin.popupCenter({
			...
			content: '/business/eam/asset_handle/asset_handle_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
