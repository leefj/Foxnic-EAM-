...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('iot-equipment-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('iot-equipment-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'iot-equipment-form-data-win');
			if(area==null) return;
			admin.putTempData('iot-equipment-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			el: "productId",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("产品",'','cmp:form'),
			filterable: true,
			paging: true,
			pageRemote: true,
			layVerify: 'required',
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("productId",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					if(window.pageExt.form.selectBoxDataTransform) {
						opts.push(window.pageExt.form.selectBoxDataTransform("productId",{data:data[i],name:data[i].name,value:data[i].id,selected:(defaultValues.indexOf(data[i].id)!=-1 || defaultIndexs.indexOf(""+i)!=-1)},data[i],data,i));
					} else {
		...
		fox.renderSelectBox({
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					if(!data[i]) continue;
					if(window.pageExt.form.selectBoxDataTransform) {
						opts.push(window.pageExt.form.selectBoxDataTransform("groupCode",{data:data[i],name:data[i].label,value:data[i].code,selected:(defaultValues.indexOf(data[i].code)!=-1 || defaultIndexs.indexOf(""+i)!=-1)},data[i],data,i));
					} else {
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('iot-equipment-form-data');
		}
		...
		if (hasData) {
			...
			//设置  产品 设置下拉框勾选
			fox.setSelectValue4QueryApi("#productId",formData.iotProduct);
			//设置  分组 设置下拉框勾选
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('iot-equipment-form-data');
		}
		return rawFormData;
	}
	...
	function getFormData() {
		...
		//获取 产品 下拉框的值
		data["productId"]=fox.getSelectedValue("productId",false);
		//获取 分组 下拉框的值
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('iot-equipment-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('iot-equipment-form-data-win',this); });
