...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-eam/eam-device-sp-rcd";
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'spId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('备件'), templet: function (d) { return templet('spId' ,fox.joinLabel(d.deviceSp,"name",',','','spId'),d);}}
					,{ field: 'operId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('使用人') , templet: function (d) { return templet('operId',fox.getProperty(d,["operUser","name"],0,'','operId'),d);} }
	...
	function refreshTableData(sortField,sortType,reset) {
		...
		var value = {};
		value.spCode={ inputType:"button",value: $("#spCode").val() ,fuzzy: true,splitValue:false,valuePrefix:"",valueSuffix:"" };
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('eam-device-sp-rcd-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('eam-device-sp-rcd-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'备件记录'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'备件记录'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('eam-device-sp-rcd-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				top.layer.load(2);
				top.layer.load(2);
				admin.post(getByIdURL, { id : data.id }, function (data) {
					...
					if(data.success) {
						admin.putTempData('eam-device-sp-rcd-form-data-form-action', "edit",true);
				...
				admin.post(getByIdURL, { id : data.id }, function (data) {
					top.layer.closeAll('loading');
					if(data.success) {
						admin.putTempData('eam-device-sp-rcd-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
				...
				top.layer.confirm(fox.translate('确定删除此'+'备件记录'+'吗？'), function (i) {
					top.layer.close(i);
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('eam-device-sp-rcd-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('eam-device-sp-rcd-form-data', data);
		var area=admin.getTempData('eam-device-sp-rcd-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('备件记录');
		...
		admin.popupCenter({
			...
			area: ["80%",height+"px"],
			type: 2,
			id:"eam-device-sp-rcd-form-data-win",
			content: '/business/eam/device_sp_rcd/device_sp_rcd_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
