...
function ListPage() {
		...
		fox.renderSelectBox({
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					if(!data[i]) continue;
					if(window.pageExt.list.selectBoxDataTransform) {
						opts.push(window.pageExt.list.selectBoxDataTransform("stockType",{data:data[i],name:data[i].label,value:data[i].code},data[i],data,i));
					} else {
	...
	function bindButtonEvent() {
			...
			switch(obj.event){
				case 'create':
					if(selected&&selected.length==1){
						...
						ps.billId=IMPORT_ID;
						admin.post("/service-eam/eam-asset-stock-goods-out/select-import-item", ps, function (res) {
							if (res.success) {
								admin.putTempData('import_data'+IMPORT_ID,selected,true);
								admin.finishPopupCenterById('eam-stock-order-list-data-win');
							} else {
								fox.showMessage(res);
							}
						}, {delayLoading:1000,elms:[$("#add-button")]});
					}else{
						top.layer.msg(fox.translate('请选择订单并只能选一个库存单','','cmp:table'), {icon: 2, time: 1500});
					}
					break;
				case 'batch-del':
					batchDelete(selected);
					break;
				case 'tool-action-detail':
					window.pageExt.list.actionDetail && window.pageExt.list.actionDetail(selected,obj);
					break;
				case 'refresh-data':
					refreshTableData();
					break;
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'库存出库'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'库存出库'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			if (layEvent === 'edit') { // 修改
				...
				top.layer.confirm(fox.translate('确定删除此'+'库存出库'+'吗？'), function (i) {
					top.layer.close(i);
