...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('eam-stock-inventory-asset-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('eam-stock-inventory-asset-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'eam-stock-inventory-asset-form-data-win');
			if(area==null) return;
			admin.putTempData('eam-stock-inventory-asset-form-area', area);
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('eam-stock-inventory-asset-form-data');
		}
		...
		if (hasData) {
			...
			//设置  盘点状态 设置下拉框勾选
			fox.setSelectValue4Enum("#inventoryStatus",formData.inventoryStatus,SELECT_INVENTORYSTATUS_DATA);
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('eam-stock-inventory-asset-form-data');
		}
		return rawFormData;
	}
	...
	function getFormData() {
		...
		//获取 盘点状态 下拉框的值
		data["inventoryStatus"]=fox.getSelectedValue("inventoryStatus",false);
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('eam-stock-inventory-asset-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('eam-stock-inventory-asset-form-data-win',this); });
