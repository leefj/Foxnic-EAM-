...
function ListPage() {
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'applyOrgId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('申请部门') , templet: function (d) { return templet('applyOrgId',fox.getProperty(d,["applyOrg","fullName"],0,'','applyOrgId'),d);} }
					,{ field: 'purchaseUserId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('采购人') , templet: function (d) { return templet('purchaseUserId',fox.getProperty(d,["purchaseUser","name"],0,'','purchaseUserId'),d);} }
					...
					,{ field: 'applyContent', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('采购说明') , templet: function (d) { return templet('applyContent',d.applyContent,d);}  }
					,{ field: 'applyDate', align:"left", fixed:false, hide:false, sort: true   ,title: fox.translate('采购日期') ,templet: function (d) { return templet('applyDate',fox.dateFormat(d.applyDate,"yyyy-MM-dd"),d); }  }
	...
	function refreshTableData(sortField,sortType,reset) {
		...
		value.assetCheck={ inputType:"radio_box", value: getSelectedValue("#assetCheck","value"), label:getSelectedValue("#assetCheck","nameStr") };
		var ps={searchField:"$composite"};
		if(window.pageExt.list.beforeQuery){
	...
	function initSearchFields() {
		...
		fox.renderSelectBox({
			...
			transform:function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					if(window.pageExt.list.selectBoxDataTransform) {
						opts.push(window.pageExt.list.selectBoxDataTransform("assetCheck",{data:data[i],name:data[i].text,value:data[i].code},data[i],data,i));
					} else {
						opts.push({data:data[i],name:data[i].text,value:data[i].code});
					}
				}
	...
	function bindButtonEvent() {
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'采购申请'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'采购申请'+'吗？'), function (i) {
                top.layer.close(i);
