...
function ListPage() {
	...
	const queryURL=moduleURL+'/query-select-paged-list-by-import';
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			var selected=getCheckedList(null);
			var selectedId=getCheckedList("id");
			...
			switch(obj.event){
				case 'create':
					if(selected&&selected.length==1){
						...
						var ps={};
						ps.id=JSON.stringify(selectedId);
						ps.importType=IMPORT_TYPE;
						ps.billId=IMPORT_ID;
						admin.post("/service-eam/eam-purchase-apply/select-import-item", ps, function (res) {
							if (res.success) {
								admin.putTempData('import_data'+IMPORT_ID,selected,true);
								admin.finishPopupCenterById('eam-purchase-order-list-data-win');
							} else {
								fox.showMessage(res);
							}
						}, {delayLoading:1000,elms:[$("#add-button")]});
					}else{
						top.layer.msg(fox.translate('请选择订单并只能选一个订单','','cmp:table'), {icon: 2, time: 1500});
					}
					break;
				case 'batch-del':
