...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('eam-inspection-task-abnormal-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('eam-inspection-task-abnormal-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'eam-inspection-task-abnormal-form-data-win');
			if(area==null) return;
			admin.putTempData('eam-inspection-task-abnormal-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			el: "taskId",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("任务名称",'','cmp:form'),
			filterable: true,
			paging: true,
			pageRemote: true,
			on: function(data){
			...
			//转换数据
			searchField: "planName", //请自行调整用于搜索的字段名称
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('eam-inspection-task-abnormal-form-data');
		}
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('eam-inspection-task-abnormal-form-data');
		}
		return rawFormData;
	}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('eam-inspection-task-abnormal-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('eam-inspection-task-abnormal-form-data-win',this); });
