    ...
    var list={
        ...
        beforeInit:function () {
            ...
            if(ROLE=="employee"){
                ...
                operHtml=operHtml.replace(/lay-event="process-rcd"/i, "style=\"display:none\"")
                ...
                operHtml=operHtml.replace(/lay-event="edit"/i, "style=\"display:none\"")
                operHtml=operHtml.replace(/lay-event="view"/i, "style=\"display:none\"")
                operHtml=operHtml.replace(/lay-event="cancel-rcd"/i, "style=\"display:none\"")
                document.getElementById("tableOperationTemplate").innerHTML=operHtml;
            }
        },
        /**
        ...
        afterQuery : function (data) {
            ...
            for (var i = 0; i < data.length; i++) {
                //如果审批中或审批通过的不允许编辑
                if (data[i].status == "cancel"||data[i].status == "finish") {
                    fox.disableButton($('.cancel-rcd-button').filter("[data-id='" + data[i].id + "']"), true);
                    fox.disableButton($('.process-rcd-button').filter("[data-id='" + data[i].id + "']"), true);
                }
            }
        },
        /**
        ...
        processRcd:function (data){
            console.log('processRcd',data);
        },
        cancelRcd:function (ps){
            console.log('cancelRcd',ps);
            var btnClass="cancel-rcd-button"
            var url="cancel";
