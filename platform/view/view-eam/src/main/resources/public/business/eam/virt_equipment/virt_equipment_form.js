...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('iot-virt-equipment-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('iot-virt-equipment-form-data'));
		}
	...
	function adjustPopup(arg) {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'iot-virt-equipment-form-data-win');
			if(area==null) return;
			admin.putTempData('iot-virt-equipment-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			el: "ownerId",
			radio: true,
			tips: fox.translate("请选择",'','cmp:form')+fox.translate("所属产品",'','cmp:form'),
			filterable: true,
			paging: true,
			pageRemote: true,
			layVerify: 'required',
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("ownerId",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					if(window.pageExt.form.selectBoxDataTransform) {
						opts.push(window.pageExt.form.selectBoxDataTransform("ownerId",{data:data[i],name:data[i].name,value:data[i].id,selected:(defaultValues.indexOf(data[i].id)!=-1 || defaultIndexs.indexOf(""+i)!=-1)},data[i],data,i));
					} else {
		...
		form.on('radio(triggerType)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=triggerType]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("triggerType",data,checked);
		});
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('iot-virt-equipment-form-data');
		}
		...
		if (hasData) {
			...
			//设置  所属产品 设置下拉框勾选
			fox.setSelectValue4QueryApi("#ownerId",formData.iotProduct);
	...
	function getRawFormData() {
		if(!rawFormData) {
			rawFormData = admin.getTempData('iot-virt-equipment-form-data');
		}
		return rawFormData;
	}
	...
	function getFormData() {
		...
		//获取 所属产品 下拉框的值
		data["ownerId"]=fox.getSelectedValue("ownerId",false);
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('iot-virt-equipment-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('iot-virt-equipment-form-data-win',this); });
