...
- SQL文件:自行从一键安装部署中提取  ( **导入时必须使用 source 命令导入！！！！！** )。
- 社区版本一键安装是最新全的，也是最全的（全部都在），数据库并未有删除操作，需要全功能版，自行通过调试yml权限机制，和RBCA权限机制调试，怎么调看文档视频去
- 折腾源代码开发经验建议三年以上，新人绕道！
- 如果这些IDEA，基础环境配置，提取SQL都做不到，别折腾源代码了，老老实实用一键安装，群主也很忙，不用来问群主，问文言一心去。
